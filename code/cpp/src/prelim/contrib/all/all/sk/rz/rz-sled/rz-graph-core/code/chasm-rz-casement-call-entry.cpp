...
#include "chasm-rz-casement-call-entry.h"
...
ChasmRZ_Casement_Call_Entry::ChasmRZ_Casement_Call_Entry(int call_id, QString prefix) //caon_ptr<ChasmRZ_Node> parent_entry_node)
...
caon_ptr<ChasmRZ_Casement_Call_Entry> ChasmRZ_Casement_Call_Entry::unwind()
{
 if(parent_entry_node_)
  return parent_entry_node_->chasm_rz_casement_call_entry();
 return nullptr;
}

caon_ptr<ChasmRZ_Casement_Call_Entry> ChasmRZ_Casement_Call_Entry::unwind(int& unwind_count)
{
 if(parent_entry_node_)
 {
  ++unwind_count;
...
void ChasmRZ_Casement_Call_Entry::debug_check_entry(ChasmRZ_Frame* fr, const ChasmRZ_Connectors& conn)
{
...
caon_ptr<ChasmRZ_Node> ChasmRZ_Casement_Call_Entry::get_next_entry_node()
{
...
caon_ptr<ChasmRZ_Node> ChasmRZ_Casement_Call_Entry::get_next_entry_node(
  caon_ptr<ChasmRZ_Casement_Call_Entry>& next_call_entry, int& unwind_count)
{
 ...
 {
  if(ref_node_->chasm_rz_casement_call_entry() == this)
  {
   unwind_count = 1;
   next_call_entry = unwind(unwind_count);
   return ref_node_;
  ...
  {
   unwind_count = 0;
   next_call_entry = ref_node_->chasm_rz_casement_call_entry();
   return ref_node_;
  }
 }
 else
...
caon_ptr<ChasmRZ_Node> ChasmRZ_Casement_Call_Entry::get_parent_entry_node_unwind()
{
 if(!ref_node_)
...
caon_ptr<ChasmRZ_Node> ChasmRZ_Casement_Call_Entry::get_node()
{
 return self_node_;
}
