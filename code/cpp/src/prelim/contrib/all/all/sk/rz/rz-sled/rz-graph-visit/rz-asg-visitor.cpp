...
caon_ptr<RZ_Graph_Embed_Token> RZ_ASG_Visitor::current_embed_rename_token(caon_ptr<tNode> node)
{
 if(node)
 {
  ...
  if(caon_ptr<tNode> n = Qy.Run_Embed_Rename(in_Cf node))
  {
...
{
 // // explain this...
 caon_ptr<tNode> en = Qy.Run_Call_Sequence(in_Cf node);
 if(en)
 {
  CAON_PTR_DEBUG(tNode ,en)
  return Qy.Run_Block_Entry(in_Cf en);
 }
 return nullptr;
}
...
{
 if(caon_ptr<ChasmRZ_Node> nn = Qy.Symbol_To_Scope(in_Cf n))
 {
...
caon_ptr<RZ_ASG_Visitor::tNode> RZ_ASG_Visitor::data_entry_from_call_entry(caon_ptr<tNode> node)
{
 if(node)
 {
  CAON_PTR_DEBUG(tNode ,node)
  node->debug_connections();
  return Qy.Run_Data_Entry(in_Cf node);
 }
 return nullptr;
}
...
caon_ptr<RZ_ASG_Visitor::tNode> RZ_ASG_Visitor::find_block_entry_node(caon_ptr<tNode> node, caon_ptr<RZ_ASG_Block_Info>& rbi)
{
 ...
 {
  node->debug_connections();
  if(caon_ptr<ChasmRZ_Node> binode = Qy.Block_Info(in_Cf node))
  {
 ...
 RZ_ASG_Visitor::embed_redirect_node(caon_ptr<tNode> node)
{
 if(node)
 {
  return Qy.Run_Embed_Redirect(in_Cf node);
 }
 return nullptr;
}
 ...
 RZ_ASG_Visitor::find_run_block_entry(caon_ptr<tNode> node, caon_ptr<tNode>* call_entry_node)
{
 ...
 if(node)
 {
  result = Qy.Run_Block_Entry(in_Cf node);
 }
 if(call_entry_node)
 {
  if(result)
  {
   *call_entry_node = Qy.Run_Call_Entry(in_Cf result);
  }
  else
 ...
 RZ_ASG_Visitor::find_call_entry(const tNode& node, caon_ptr<ChasmRZ_Call_Entry>& rce)
{
 ...
 {
  return Qy.Run_Call_Entry(in_Cf pnode);
 }
 else if(caon_ptr<tNode> next_node = Qy.Run_Call_Entry(in_Cf pnode))
 {
 ...
 RZ_ASG_Visitor::find_data_entry(const tNode& node, caon_ptr<ChasmRZ_Call_Entry>& rce)
{
 ...
 if(rce = node.chasm_rz_call_entry())
 {
  ...
  return Qy.Run_Data_Entry(in_Cf pnode);
 }
 else if(caon_ptr<tNode> next_node = Qy.Run_Call_Entry(in_Cf pnode))
 {
 ...
 RZ_SRE_Result_State& result_state, bool expression_flag, bool non_sequence)
{
 ...
 if(!non_sequence)
 {
  ...
  else if(result = Qy.Run_Call_Entry(in_Cf start_node))
  {
  ...
  else if(result = Qy.Run_Data_Entry(in_Cf start_node))
  {
   result_state.read_table_state = RZ_Read_Table_State::Expression_Sequence;
   result_state.advance_token_state = RZ_Read_Table_Advance_Token_State::Token_Loaded;
   result_state.advance_graph_state = RZ_Read_Table_Advance_Graph_State::Call_Sequence;
  ...
  else if(result = Qy.Run_Data_Leave(in_Cf start_node))
  {
 ...
 if(result = Qy.Run_Block_Entry(in_Cf start_node))
 {
 ...
 if(result = Qy.Run_Cross_Sequence(in_Cf start_node))
 {
  if(caon_ptr<ChasmRZ_Call_Entry> rce = result->chasm_rz_call_entry())
  {
...
{
 if(caon_ptr<tNode> dl_node = Qy.Run_Data_Leave(in_Cf &n) )
 {
  ...
  QString& explanation)
{
 ...
 caon_ptr<tNode> result = Qy.Run_Nested_Do_Map_Block_Entry_Rewind(in_Cf node);
 ...
 if(result)
 {
  ...
  caon_ptr<tNode> block_entry_node = Qy.Run_Block_Entry(in_Cf result);
  CAON_PTR_DEBUG(tNode ,block_entry_node)
  if(caon_ptr<tNode> cen = Qy.Run_Call_Entry(in_Cf result))
  {
   ...
   else if(parent_node = Qy.Parent_Block_Map(in_Cf ben))
   {
    ...
    if(caon_ptr<ChasmRZ_Block_Entry> rbe = ben->chasm_rz_block_entry())
    {
     ...
     if(caon_ptr<tNode> sen = rbe->statement_entry_node())
     {
      ...
      CAON_DEBUG_NOOP
      cross_node = Qy.Run_Cross_Sequence(in_Cf sen);
     }
    }
  ...
  RZ_ASG_Visitor::find_block_continue_node(caon_ptr<tNode> node, caon_ptr<tNode>& block_entry_node)
{
 ...
 if(!node)
   return nullptr;
 node->debug_connections();
 caon_ptr<tNode> result = Qy.Run_Nested_Block_Continue(in_Cf node);
 if(result)
 {
  block_entry_node = Qy.Run_Block_Entry(in_Cf result);
 }
 return result;
}
 ...
 if(caon_ptr<ChasmRZ_Call_Entry> rce = node->chasm_rz_call_entry())
 {
  ...
  if(bl_node)
  {
   ...
   if(caon_ptr<ChasmRZ_Block_Entry> rbe = bl_node->chasm_rz_block_entry())
   {
    ...
    {
     CAON_PTR_DEBUG(tNode ,sen)
     if(caon_ptr<tNode> result = Qy.Run_Cross_Sequence(in_Cf sen))
     {
      CAON_PTR_DEBUG(tNode ,result)
      return result;
     }
  ...
  else if(par)
  {
   if(par != ref)
   {
    if(caon_ptr<ChasmRZ_Node> next_node = Qy.Run_Cross_Sequence(in_Cf par))
    {
...
caon_ptr<RZ_ASG_Block_Info> RZ_ASG_Visitor::current_block_info(caon_ptr<tNode> node)
{
 if(node)
 {
  CAON_PTR_DEBUG(ChasmRZ_Node ,node)
  node->debug_connections();
  if(caon_ptr<tNode> n = Qy.Block_Info(in_Cf node))
  {
   return n->block_info();
  }
 }
...
RZ_ASG_Visitor::Next_Node_Premise RZ_ASG_Visitor::get_next_node(caon_ptr<ChasmRZ_Node> start_node, caon_ptr<ChasmRZ_Node>& result)
{
 ...
 if(result = Qy.Run_Function_Def_Entry(in_Cf start_node))
 {
...
{
 return Qy.Run_Call_Sequence(in_Cf node);
}
...
caon_ptr<ChasmRZ_Node> RZ_ASG_Visitor::get_expression_review_node_from_entry_node(caon_ptr<ChasmRZ_Node> entry_node)
{
 ...
 return Qy.Element_Review(in_Cf entry_node);
}
  ...
  caon_ptr<ChasmRZ_Node> statement_entry_node)
{
 ...
 return Qy.Assignment_Annotation(in_Cf statement_entry_node);
}
...
{
 CAON_PTR_DEBUG(ChasmRZ_Node ,statement_entry_node)
 return Qy.Element_Association(in_Cf statement_entry_node);
}
...
{
 if(caon_ptr<tNode> result = Qy.Raw_ASG_Paste(in_Cf node))
 {
  CAON_PTR_DEBUG(tNode ,result)
  return result;
 }
...
{
 if(caon_ptr<ChasmRZ_Node> bin = Qy.Block_Info(in_Cf node))
 {
...
{
 caon_ptr<tNode> result = Qy.Run_Call_Entry(in_Cf sn);
 if(result)
 {
  CAON_PTR_DEBUG(tNode ,result)
  if(caon_ptr<ChasmRZ_Call_Entry> rce = result->chasm_rz_call_entry())
  ...
  caon_ptr<tNode>* node_to_change)
{
 ...
 {
    // //?
  if(caon_ptr<ChasmRZ_Node> nn = Qy.Run_Nested_Do_Map_Block_Entry(in_Cf &start_node))
  {
  ...
  else if(n = Qy.Run_Data_Entry(in_Cf &start_node))
  {
  ...
  const ChasmRZ_Connectors& qtok)
{
 ...
 if(current_node = Qy.Run_Call_Sequence(in_Cf &start_node))
 {
 ...
 tNode& start_node, const ChasmRZ_Connectors& qtok, caon_ptr<tNode>* node_to_change)
{
 if(caon_ptr<ChasmRZ_Block_Entry> rbe = start_node.chasm_rz_block_entry())
 {
  if(caon_ptr<tNode> call_start_node = Qy.Run_Call_Entry(in_Cf &start_node))
  {
   ...
   {
    CAON_PTR_DEBUG(tNode ,node)
    while(caon_ptr<tNode> next_node = Qy.Run_Cross_Sequence(in_Cf node))
    {
...
{
 pre_entry_node.swap_relation(&in_Cf qtok, &head_node, &function_node);

 if(caon_ptr<ChasmRZ_Node> retval_node = Qy.Retval_Follow(in_Cf &head_node) )
 {
  head_node.delete_relation(&in_Cf Qy.Retval_Follow, retval_node);
 ...
 previous_node.delete_relation(&in_Cf Qy.Run_Call_Sequence, &function_node);
 caon_ptr<tNode> next_node;
 if(next_node = Qy.Run_Call_Sequence(in_Cf &function_node))
 {
  function_node.delete_relation(&in_Cf Qy.Run_Call_Sequence, next_node);
  &previous_node << Cf/Qy.Run_Call_Sequence >> next_node;
 }
 else if(next_node = Qy.Run_Call_Entry(in_Cf &function_node))
 {
  function_node.delete_relation(&in_Cf Qy.Run_Call_Entry, next_node);
  &previous_node << Cf/Qy.Run_Call_Entry >> next_node;
 }

 if(caon_ptr<tNode> cross_node = Qy.Run_Cross_Sequence(in_Cf &head_node))
 {
  head_node.delete_relation(&in_Cf Qy.Run_Cross_Sequence, cross_node);
  &function_node << Cf/Qy.Run_Cross_Sequence >> cross_node;
 }
 else if(cross_node = Qy.Run_Block_Entry(in_Cf &head_node))
 {
  head_node.delete_relation(&in_Cf Qy.Run_Block_Entry, cross_node);
  ...
  caon_ptr<ChasmRZ_Call_Entry> carried_rce, int depth, tNode& function_node)
{
 ...
 while(previous_node)
 {
  ...
  caon_ptr<tNode> next_node;
  if(next_node = Qy.Run_Call_Sequence(in_Cf previous_node))
  {
   ++count;
   previous_node = next_node;
  }
  else if(next_node = Qy.Run_Block_Entry(in_Cf previous_node))
  {
   previous_node = normalize_nested_block(previous_node, next_node, Qy.Run_Block_Entry);
   ++count;
  }
  else if(next_node = Qy.Run_Call_Entry(in_Cf previous_node))
  {
   CAON_PTR_DEBUG(ChasmRZ_Node ,next_node)
...
{
 if(caon_ptr<ChasmRZ_Node> don = Qy.Run_Cross_Do(in_Cf n))
 {
  ...
  caon_ptr<tNode> previous_node, caon_ptr<tNode> function_node)
{
 ...
 caon_ptr<ChasmRZ_Node> pnnode = Qy.Run_Cross_Sequence(in_Cf previous_node);
 caon_ptr<ChasmRZ_Node> fnnode = Qy.Run_Cross_Sequence(in_Cf function_node);
 ...
 else if(next_node = Qy.Run_Block_Entry(in_Cf function_node))
 {
...
QString RZ_ASG_Visitor::call_entry_label(caon_ptr<ChasmRZ_Node> n)
{
 QString result;
 CAON_PTR_DEBUG(ChasmRZ_Node ,n)
 if(caon_ptr<ChasmRZ_Node> ln = Qy.Run_Map_Key_Label(in_Cf n))
 {
  ...
  caon_ptr<tNode> function_node, const ChasmRZ_Connectors& qtok)
{
 ...
 if(caon_ptr<tNode> en_node = Qy.Run_Cross_Sequence(in_Cf function_node))
 {
...
{
 CAON_PTR_DEBUG(tNode ,node)
 return Qy.Type_Indicator(in_Cf node);
}
...
QString RZ_ASG_Visitor::type_expression_from_node(caon_ptr<tNode> node)
{
 ...
 CAON_PTR_DEBUG(ChasmRZ_Node ,node)
 node->debug_connections();
 if(caon_ptr<ChasmRZ_Node> ce_node = Qy.Run_Call_Entry(in_Cf node))
 {
  CAON_PTR_DEBUG(ChasmRZ_Node ,ce_node)
  ce_node->debug_connections();
  node = Qy.Run_Cross_Sequence(in_Cf ce_node);
  CAON_PTR_DEBUG(ChasmRZ_Node ,node)
  while(node)
  {
   ...
   node = Qy.Run_Call_Sequence(in_Cf node);
  }
 }
 if(result.endsWith(' '))
  result.chop(1);
...
{
 CAON_PTR_DEBUG(tNode ,node)
 caon_ptr<tNode> result = Qy.Run_Data_Entry(in_Cf node);
 return result;
}
...
caon_ptr<tNode> RZ_ASG_Visitor::anticipate_run_call(tNode& start_node)
{
 if(caon_ptr<ChasmRZ_Call_Entry> rce = start_node.chasm_rz_call_entry())
 {
  ...
  if(caon_ptr<tNode> node = Qy.Run_Call_Entry(in_Cf &start_node))
  {
   ...
   if(caon_ptr<tNode> block_entry_node = Qy.Run_Nested_Block_Entry(in_Cf &start_node))
   {
    valuer_->enter_new_lexical_scope();
...
{
 caon_ptr<tNode> result = Qy.Run_Cross_Sequence(in_Cf node);
 return result;
}
  ...
  RZ_ASG_Result_Holder& rh)
{
 ...
 if(n = Qy.Run_Call_Sequence(in_Cf &start_node))
 {
  CAON_PTR_DEBUG(tNode ,n)
  anticipate_nested_run_calls(*n, rh);
 }
 if(n = Qy.Run_Cross_Sequence(in_Cf &start_node))
 {
  anticipate_nested_run_calls(*n, rh);
 }

 if(n = Qy.Run_Block_Entry(in_Cf &start_node))
 {
  anticipate_run_call(*n);
 }
 else if(n = Qy.Run_Cross_Block(in_Cf &start_node))
 {
  anticipate_run_call(*n);
 }
}
