   ...
   raw_text_(raw_text), line_number_(line_number)
{
 if(raw_text_ == "do")
 {
  if( (prefix.size() == 0) && (suffix.size() == 0) )
  {
   flags.is_do = true;
   special_token_ =
    ChasmRZ_Code_Representation::check_special_token(raw_text_, false);
   return;
  }
 }
 ...
 special_token_ =
  ChasmRZ_Code_Representation::check_special_token(raw_text_, parse_special_token);

}

QString ChasmRZ_Token::special_token_kind_string() const
{
 switch(special_token_kind())
 {
 case ChasmRZ_Code_Representation::Special_Token_Kind::Arrow:
  return "Arrow";
 ...
 case ChasmRZ_Code_Representation::Special_Token_Kind::Normal:
  return "Normal";

 case ChasmRZ_Code_Representation::Special_Token_Kind::N_A:
  return "N_A";
...
QString ChasmRZ_Token::clasp_string_value() const
{
...
void ChasmRZ_Token::resolve_arrow_prefix(QString prefix, QString raw_text)
{
 Flags = 0;
...
//      keyword, etc.
void ChasmRZ_Token::inital_check_numeric()
{
...
QString ChasmRZ_Token::get_asg_out() const
{
 QString result = special_token_.asg_name;
 if(result.isEmpty())
 {
  result = raw_text_;
 }
...
QString ChasmRZ_Token::string_summary(int length)
{
