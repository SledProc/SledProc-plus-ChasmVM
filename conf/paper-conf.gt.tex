...
content (e.g., when seeing it rendered in a \eA{PDF} viewer). Such
...
the \eASomeLower{Poppler} library has a \eS{text} method of
...
these alternatives, plus the fact that the \eA{JSON}
markup appeared sometimes to incorrectly parse
the strings \mdash{} i.e., breaking the character-sequence
...
\citePage{LucyLuWangEtAl}(8)}\thispagestyle{empty}
}% end paragraph 
...
\eA{PDF} text representation, it would be
reasonable for \forcehypertarget{poppler-adj}
\eA{PDF} viewers to run text searches first through more-precise data about
text content \mdash{} if \eA{PDF} files provide as much \mdash{} in lieu of
\eA{PDF}-internal text objects. For example, many \eA{PDF} files are
currently produced from \eA{XML} formats such as
\eA{JATS} or \eA{BITS}; if these intermediate assets (or at least
...
(which I've mentioned in footnote \lhRef{poppler-adj}{3}).
...
format \mdash{} potentially extended to \q{full-text search} with
associated text documents, via query specifications
geared to text content (\textit{cf.} \citeLabel{ChavdarBotevEtAl}, for example). In other scenarios, an Executable Research Object
...
conducive to advanced research \mdash{} the idea of Executable Research Objects is not necessarily
...
anyone not familiar with or able to run the \q{professional} software).
}% end paragraph 
...
identify potential \eA{VM} use cases. One obvious example is deserializing data sets.
...
canonical languages like \eA{XML} or \eA{JSON}, but even in those cases dedicated code is
...
alternatives such as \eA{RDF}, multi-dimensional arrays, multi-value \eA{SQL}, and so on).
}% end paragraph 
...
to interactive screen areas \mdash{} here, individual projects
...
instance, notebook tabs \mdash{} whose
...
\eI{ctrl} or \eI{alt}. When constructing
...
into the \eA{VM}'s instruction roster.\vspace{11pt}
}% end paragraph 
...
not just a realization or dialect of \eA{XML}) \mdash{} have been proposed as more faithful
...
the representation of \textit{text} content, not generic
...
approximately captured by \q{topomorphic regimes} \textit{in toto} of
...
\q{Keynesian economics, and macroeconomic theory} and so on.
...
molecular-structure files in formats such as \eA{PDB}, \eA{MOL}, \eA{CML} (Chemical Markup
Language), or similar standards. Those files may then be
read \mdash{} and converted to \eA{GUI} displays \mdash{}
...
\eA{PDB} accession numbers, launch-requests for external
...
procedures for \eA{XYZ} to \eI{lat}/\eI{lon} conversion,
...
(albeit maybe \textit{stored} with \textit{one} byte).
...
formats, \textit{internal} (object-based) encoding, and various result
...
as necessary) are comparatively straightforward to compile \mdash{} these
...
contrast is as follows: \q{procedures} can typically be
...
\p.1{%
Another idiosyncratic aspect of the demo \eA{VM} is what the code
...
be default \textit{constructed}} A \eA{VM} compiler and
