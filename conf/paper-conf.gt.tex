...
\eS{if}-expression like \eH{x.size() == y.size()} as testing the exact same state of affairs
...
This term hopefully connotes how the result of \eH{x.size() == y.size()} is not just a boolean
\eS{true} or \eS{false}, but a more substantial object that \textit{evaluates} to boolean
...
to evaluate not only to a boolean return value but to a \q{reified} object in this sense.
}% end paragraph 

\p.1{%
When an \eH{if}/\eH{then} construction does involve a \textit{reified} boolean I assume
...
conventions. Conceptually, \eH{if-then-else}, \eH{do-while}, etc., are not
...
Moreover, sometimes the caller \textit{wants} a
...
\enumsItem{}   The primary goal of the callee is to initialize or modify
that object
\enumsItem{}   The object is intended mostly to provide information the callee
...
\p.1{%
In \eA{C++}, moreover, pass-by-reference serves as a workaround
...
was only declared) or else \q{re-initializations} of objects/values
...
that (unlike the typical case) may be \textit{either} input or output.
For sake of discussion, suppose we coin the phrase \q{Janus pointer}
...
possible types \mdash{} comparable in cardinality
to a range of ordinary values \mdash{} then the
...
characterization asserted relative to \eH{y}.
}% end paragraph 
...
value is exactly \eH{n}. The check for a
...
(given an \q{instantiation equals block} paradigm)
than as \textit{de facto} types attributed to their bearers.
Not only does this approach clarify
dependent-typing possibilities, it also
indicates how template metaprogramming
might be supported in a (comparatively lightweight) \eA{VM} context.\enlargethispage{1em}
}% end paragraph 
...
\p.1{%
Consider the case of \eA{2D} graphics. There are many (e.g.)
...
A \eASomeLower{Qt} rectangle, for example (in contexts like \eS{QGraphicsScene}s)
