...
\noindent{}\pgl{}In data sharing and publishing
contexts such as \q{Executable Research Objects},
...
into a common search, indexing, and \eA{GUI} framework.
...
data, and according to \eA{FAIR} (Findable,
Accessible, Interopable, Reusable) principles
...
\p.1{%
In the context of \q{publishing technology}, one would traditionally
understand a \q{publication} to be a natural-language document
(e.g. an article, book, or book chapter). But several
more recent trends in publishing and science have complicated
this picture. For example: the idea of a \q{replication crisis}
\mdash{} concernts that results of influential experiments in natural and social
sciences have proven difficult to reproduce during updated studies.
Reaction to this \q{crisis}
spurred the goal of \q{data transparency} and initiatives to
document more rigorously the raw data, methodology, and analytic
details behind published findings. Such initiatives in turn
led to \q{Minimum Information} standards (e.g.,
Minimum Information for Biological and Biomedical Investigations, or
\eA{MIBBI}, which encompasses several dozen more specific specifications)
and to mixed data/code/multimedia formats such as \eA{FAIR}sharing
(Findable, Accessible, Interoperable, Reusable) and
Research Objects (including \q{Executable Research Objects},
\q{Executable Papers},
Research Object \q{Bundles}, Research \q{Compendia},
or \mdash{} as with the name of one US \q{Data Infrastructure Building Blocks} (\eA{DIBBs})
initiative \mdash{} \q{the Whole Tale}.\docFootnote{For the \q{compendium}
model, see \citeLabel{LimorPeerEtAl}. For \q{executable papers},
\citeLabel{RudolfStrijkersEtAl}. For \eA{FAIR}data,
\citeLabel{NickGarabedianEtAl}. For the \q{Whole Tale}, \citeLabel{BertramLudascherEtAl}.}
}% end paragraph 
...
\mdash{} readers are most likely to learn about data sets or Research
Objects via publications that introduce them \mdash{} but in a Research
...
intrinsic to text documents insofar as, to begin with, the various components
...
\p.1{%
Relatedly, researchers have explored the use of text mining
and Natural Language Processing tools to promote
the dissemination of scientific work. In principle, say,
computational methods can highlight which additional
papers might benefit a particular researcher, given ones
they have already cited or written themselves. As one example,
the \CORDOneNine{} corpus was curated by the
Allen Institute for Artificial Intelligence, early in the Covid-19 pandemic,
as a large repository of open-access materials about \eA{SARS} and Coronaviruses
...
of fitting different pieces of a puzzle together \mdash{} especially
early on, when the morphology, infectious mechanisms, treatment
options, and vaccination strategies of \eASomeLower{SARS-Cov-2} were still
under investigation \mdash{} a computationally analyzable corpus
held potential for scientists with one specialization to
find useful information in articles they might not
discover by mere chance.\docFootnote{For a general introduction
to \CORDOneNine{} see \citeLabel{LucyLuWangEtAl}. Several other
publications have described analysis that \textit{used}
...
analysis of \CORDOneNine{} in the second half of
Chapter 3 of \citeLabel{NCDataIntegration}.}
}% end paragraph 

\p.1{%
In effect, the goal of \CORDOneNine{} was not just to free up
...
of those publications \mdash{} \eA{PDF} \mdash{} is flawed from a
computational perspective because the internal
\eA{PDF} representation of natural language often differs
from how human readers would transcribe the same
content when seeing it rendered in a \eA{PDF} viewer. Such
details as ligatures, headers/footers, hyphenation, and
font-face variations can introduce artifacts in
text extraction (or \q{scraping}) from \eA{PDF} files.\docFootnote{\label{fn:poppler}To clarify these remarks: \eA{PDF} code libraries typically
have some procedure that can be called to obtain text
from one page and/or window area. In \eA{C++}, for example,
the \eASomeLower{Poppler} library has a \eAAllLower{text} method of
\eS{Poppler::Page} which optionally takes a rectangular subarea (this allows
scraper to eliminate headers and footers, for instance).
The strings returned by this method, however, will
usually differ from the input sources captured as
\eA{JATS}, for example. The most straightfoward
contast would be among hyphenated words, but other anomalies
may occur, such as unexpected space characters distributed
between letters of words in special fonts (small spaces
may be present in the internal \eA{PDF} text that are invisible to
human readers, but methods like \eS{Page::text} have no mechanism
to report different sizes of text characters. Likewise, depending
on the library being used, text-extraction routines
may or may not \q{unpack} ligatures like \q{ffi} to their
constituent letters.} In addition
to problems with \q{ordinary} language, encoding errors
also arose in the context of \q{special} character
strings such as organic or chemical formulae; the
\CORDOneNine{} documents did not use a consistent mechanism for
notating these (potentially semantically significant)
textual elements (or, at least, structured data
was not preserved in the final \eA{PDF}).\docFootnote{In my own analysis of \CORDOneNine{} mentioned above,
I gave the example of a compound \eIbox{2'-C-ethynyl},
which is also notated as \eIbox{2 0-C-ethynyl}. With
these alternatives, and also the fact that the \eA{JSON}
markup appeared in places to incorrectly parse
the strings \mdash{} i.e., breaking the formula
at the wrong location \mdash{} the corpus overall
has multiple different representations for
the same formula. This obvious obscured the
fact that each of these represent the
same concept, and indicate how semantic
annotation would be more reliable.} These problems
were openly acknowledged by the team behind \CORDOneNine{}, who
described the steps they took to (imperfectly) convert
\eA{PDF} files to a machine-readable \eA{JSON} format. They even issused
a \q{call to action} encouraging publishers to adopt a
more consistent text representation.\docFootnote{Under a section explicitly titled \q{call to action} they
counsel that \q{a number of
challenges prevent easy application of \eA{NLP}
and text mining techniques to these papers\ellFour{2pt}{2pt} [T]he
primary distribution format of scientific papers
\mdash{} \eA{PDF} \mdash{} is not amenable to text processing. The
\eA{PDF} file format is designed to share electronic documents
rendered faithfully for reading and printing, and
mixes visual with semantic information.
Significant effort is needed to coerce \eA{PDF} into a
format more amenable to text mining, such as \eA{JATS}
\eA{XML,} \eASomeLower{BioC} \ellThree{2pt}{2pt}\ or \eA{S2ORC} \eA{JSON....} [T]he
community must define and agree upon an
appropriate standard of representation.}
\citePage{LucyLuWangEtAl}(6)}\thispagestyle{empty}
}% end paragraph 
...
within \eA{PDF} documents. Given the imperfections of
\eA{PDF} text representation, it would be reasonable for \eA{PDF} viewers to
run text searches first through more-precise data about
text content \mdash{} if \eA{PDF} files provide as much \mdash{} in lieu of
\eA{PDF}-internal text objects. For example, many \eA{PDF} files in
current publishing are produced from \eA{XML} format such as
\eA{JATS} or \eA{BITS}; if such intermediate assets were embedded in
\eA{PDF} directly then they could be extracted and employed by
\eA{PDF} viewers as structured text encoding, in preference to the
more \q{semi-structured} data one may obtain from
methods such as \eS{text()} in the \eASomeLower{Poppler} library
(which I've mentioned in footnote \lfRef{fn:poppler}).
...
proper \mdash{} e.g., deriving parse-graphs or Syntactic Categories
(parts of speech) for sentence components \mdash{} is at least
tangentially connected to these trends, but the
(at best approximative) methods of \eA{NLP} are outside the
scope of topics addressed here. Instead, the instantiation of
\q{natural language} content in this present context involves
fully-deterministic encoding of textual material in the
specifically \textit{written} environment of academic and scientific
...
these might be designated as facets of \textit{discourse} rather than
...
text and to data sets, as well as \eA{GUI} components that present
them respectively in the form of visual resources.
}% end paragraph 

\p.1{%
For example, we can isolate specific areas of potential
\eA{GUI} functionality in contexts where human readers are seeing
...
\eA{VM} would be concretely utilized to implement a query language,
scripting environment, middleware tools, or any other
tactic in relation to published Research Objects. I \textit{will}
work under the assumption that authors and digital editors
seek to align with \eA{FAIR} priorities, which means in particular
that data sets are \textit{executable} with minimal external dependencies.
In other words, as much as possible, interested readers
should be able to obtain the content of an Executable Research
Object \textit{in source code} fashion and use those sources
\mdash{} e.g., by building an entry-point executable \mdash{} without having to
separately obstain and/or compile or install additional resources
...
environment typical for their language of choice \mdash{} for example,
a \eA{C++}-based bundle obviously requires a \eA{C++} compiler, and
canonical libraries such as \eAAllLower{boost} or \eASomeLower{Qt} are acceptable external
dependencies. Nonetheless, publishers should be careful to
ensure that clear instructions are given for users who need to
set up a prototypical development environment to begin with.\docFootnote{And external
dependencies should be favored which are easier to use. For instance,
if two \eA{C++} libraries provide similar functionality, developers should
comparse them based on how easy or difficult it is to install
the libaries assuming they are not already present on one's
...
have \textit{some} dependencies (at least, either compilers or scripting-language
...
distributed as Kaggle notebooks or similar), or \eAAllLower{reprozip} \citeLabel{VickyStevensEtAl} (which packages
...
Virtual Machine that can function in the former context \mdash{} in particular,
...
as source files in a mainstream programming language (\eA{C++}, \eASomeLower{Java}, etc.),
...
information encoded in a data set may be approached \textit{de facto} as a
database which accepts queries in an \eA{SQL}-like (or some flavor of \eASomeLower{NoSQL})
format \mdash{} potentially extended to \q{full-text search} \visavis{}
associated text documents. In other scenarios, an Executable Research Object
could employ something like a built-in scripting language to run
analytic code, fine-tune \eA{GUI}s, compose plugins for external scientific applications,
...
via disipline-specific applications that have analytic or visualization capabilities
conducive to advanced research \mdash{} the idea of Executable Research Object is not necessarily
to replace the tools that scientists use for their core investigations, but rather to
provide accessible code for evaluative and pedagogical purposes for
users not familiar with or able to run the \q{professional} software).
}% end paragraph 


\s|2|{Deserialization and Other User Cases}

\p.1{%
To clarify why a \eA{VM} can have nontrivial roles in a Research Object context, I will
identify potential \eA{VM} use cases. One obvious example is that of deserializing data sets.
...
canonical formats like \eA{XML} or \eA{JSON}, but even in these cases dedicated code is
needed to traverse a \eA{DOM} hierarchy, respond to \eA{SAX} events, or otherwise
...
to, say, \eA{DOM} navigation). Even a simple standard like \eA{CSV} often does not map precisely
to serialized objects, given that most aggregate values cannot be fully decomposed
into tuples of scalar data points (instead, \eA{CSV} circuitously encodes vector/array
structures, or values sampled from enumerated classes, or special-purpose fields
that are usually undefined or default-initialized and thus encoded as empty cells).\docFootnote{One example I am familiar with is the Toxic Release Inventory (\eA{TRI}) project
of the Environmental Protection Agency. The \eA{EPA} publishes annual reports,
...
dozen known harmful substances). The raw data takes the form of \eA{CSV} files
...
in the collection; still others encode text strings which have to comform
...
\eH{enum}s, boolean flags, and so forth to represent many of the
data-points. All of this obviously complicates the deserialization
process (\q{\eA{CSV}-to-Object mapping}, so to speak).}
That is, code to deserialize \eA{CSV} can be equally complex to \eA{XML} or \eA{JSON} (likewise for
alternatives such as \eA{RDF}, multi-dimensional arrays, and so on).
}% end paragraph 
...
libraries are often maintained by individual academic departments or other
...
external dependencies (or other details that make the libaries hard to build and/or
use for some people), and other issues affecting \eA{FAIR}-style accessibility.
}% end paragraph 

\p.1{%
In principle, a \eA{VM} in this context could provide a common environment for
...
The \eA{VM} instruction set could be interpreted by
multiple languages in variegated environments \mdash{} i.e., the \eA{VM} runtime itself
...
\eA{VM} runtime might be made available as a binary component (so long as it is
tested against multiple platforms, to the extent that it serves as a
\q{\eA{FAIR}} component).
}% end paragraph 

\p.1{%
Assuming that this is indeed a primary use-case, notice that the \eA{VM} would not
...
scripting format, the \eA{VM} would not primarily function as host to a
general-purpose language like \eASomeLower{Python} or \eASomeLower{JavaScript}. Instead, it could be designed
...
who downloaded it. Potentially, this involves both dedicated \eA{GUI} windows
showing details about object-collections in aggregate and also individual
objects/records (e.g., something like a \eA{GUI} \q{form} showing the values
of individual data fields in an organized layout, perhaps with list-views
or hierarchical \q{accordiaos} for non-scalar information) as well as statistical
overviews, presented via (interactive) graphics (charts, plots,
diagrams, histograms, etc.).\docFootnote{\q{Accordion} is a common term for a sort
of \eA{GUI} control that has multiple inner segments, usually stacked
...
visible.} This is another area where \eA{VM}s might be
pressed into service.
}% end paragraph 

\p.1{%
Suppose a Research Object is implemented in \eA{C++}:
although in principle all the \eA{GUI} code could be built specifically for that
...
\eA{C++} sources are included within the data set itself; in principle,
each Research Object could thereby reuse generic components but
adopt the underlying \eA{C++} code for their own purposes, given the
...
query format rather than manually rewriting \eA{C++} code. For instance,
context menus tend to need specific action options unique to the
current data set, so \q{scripts} against an embedded \eA{VM} might be
employed to populate those menus, being called somewhere after
the triggering event (a right mouse-button click or \ctrlleft{} button)
...
start shortly before the still that is depicted). If
the video actually records the run of
one computer simulation, the
user could instead be brought to \eA{GUI} controls
...
\eA{PDB}) data files that certain applications are able to use
for constructing \ThreeD{} molecular visualizations, modeling
that subtantce's atomic structure. If the \eA{PDF} viewer
...
\p.1{%
Context menus are a good case in point because, in a \eA{PDF} display,
most of a reader's attention is focused on the document and not
on \eA{GUI} controls scatterred by the edge of the window. Therefore,
functionality requested in the document's context would
typically be activated via a context menu rather than a
\eA{GUI} button or other clickable area (the menu could be
...
instance, when a user enters a term into the \eA{PDF} search
box, windows operating in a Research Object context might
well ask whether the search should be applied to
the entire package, not just the current \eA{PDF} file
(exactly how to interpret searches over, say, raw
data files depends on the data set in question). A
\eA{PDF} viewer designed specifically to embed
in Executable Research Objects could moreover adopt
a standard protocol for adding \eA{GUI} buttons/controls
to interactive screen areas where individual projects
would place clickable elements that invoke functionality
more general than just those localized to
particular areas of text/documents (and therefore
less suitable to context menus).
}% end paragraph 

\p.1{%
Indeed, \eA{GUI} programming overall evinces a variety
of common practices that are relatively
standardized and help users feel comfortable
with a dektop-style software component.
...
in a window at any one time; and
functionality is distributed amongst
screen areas of different size (for
instance, a notebook tab \mdash{} whose
roles include switching from one tab-view
to another when clicked \mdash{} also typically
have a small \eI{x}-shaped close button
...
with little effort (similary, a common
policy is to append an asterix to file
...
button. In \eA{3D} graphics and \eA{2D} (manual) image
annotation there are often multiple
possible interpretatons of actions like
...
and others rotate the arrow to a desired angle.
When certain patterns of interaction (through
...
desktop \eA{GUI}s are not just programming
concerns, but also suggest a certain \q{semantics}
for \eA{GUI} components. For example, each
...
cursor appearance, gesture logitics
...
across \eA{GUI} components is to defer certain tasks
away from the compiled code and toward dynamically
loadable scripts. For example, software can run in a
dedicated \q{demonstration} mode that employs
screen overlays to show information about
particular window controls. That functionality
could be implemented via \eA{VM} scripts orthogonal
to and developed after the main application.
}% end paragraph 


\s|2|{Text Search and Data Analyses\\{\sth}(or demonstrative summaries thereof)}

\p.1{%
Another potential \eA{VM} use case involves data analysis.
Not to imply that a dataset-embedded \eA{VM} would be part of
...
value ranges, ccordinate systems,
...
in mainstream languages, so a Research Object \eA{VM} could
supporting scripting dialects which attempt to
make code assumptions and pedagogical use-cases
as transparent as possible.\docFootnote{To put it differently, the original
analysis might be performed in \eA{C}, \eASomeLower{Python},
\eASomeLower{Matlab}, \eAAllLower{fortran}, etc., but all or some
of that code would then get reimplemented in a more
explicitly \q{pedagogical} language, as
if developers seek to explain the code to an outside party.}
}% end paragraph 

\p.1{%
Lastly, I mentioned earlier that researchers may prefer to
load data sets with external software even if \eA{FAIR} principles
imply that Research Objects can be used on their own.
So \mdash{} although authors should attempt to provide sufficient
internal resources that someone with access to a data set
can find some value just with the materials explicitly
included \mdash{} one may reasonably anticipate that some scientists will
seek out the additional capabilities of dedicated
software for their field (at least if a Research Objech
involves information of a variety where such software exists).
Data sets (without violating \eA{FAIR}) could be
...
\eA{CSV} files, but such data rendered as table cells
...
special prepatory requirements can provide plugins
...
an application to execute \eA{VM} instructions, which then
serves as middleware such that Research Object plugins
could be implemented as scripts compiling to that \eA{VM}, rather
...
I will also touch on how \eA{VM} technology could support text-related
...
\eA{C++}), then in principle queries such as \q{find the first sentence
including the name of \eI{this author}} (the author of a given bibliographic entry)
could be satisfied by calling (in this case) \eA{C++} methods.
...
\p.1{%
Other possible use-cases arise in the context of \eA{PDF} viewers,
in which could be embedded a \eA{VM} interpreter/runtime.
New functionality may then be intrduced via \eA{PDF}-related
scripts; for instance, extra context-menu options,
interactive features pertaining to figures/graphics,
customizations for \eA{PDF} annotations and comment
...
suggest that \eA{VM}s embedded in Executable Research Objects
can be useful across the different asset-types (documents,
code, raw data, graphics/multimedia). Under the assumption
that a compiler and runtime environment (bytecode interpreter)
is offerred among the data-set package, then other
components \mdash{} \eA{GUI} classes, a customized \eA{PDF} viewer, deserialization
code \mdash{} could each utilize (e.g., link against) the
\eA{VM} implementation in turn, achieving consistent scripting/query
design across the various components.
}% end paragraph 

\p.1{%
Having thereby identified the \textit{purpose} of a \eA{VM} engineered
...
structures \mdash{} both textual and data sets \mdash{} that
\eA{VM}-compiled components would likely
need to traverse/navigate, insofar as the relevant
structural details may be directly incorporated
into the \eA{VM}'s instruction roster.\vspace{21pt}
}% end paragraph 


\s|1|{Sentence Objects and Other Textual Elements}

\p.1{%
Conceptually, we assume that a Research Object
\eA{VM} will operate over a tableau of information
...
files and other reseach assets, subject to
...
\eA{XML}-based standards such as \eA{JATS} and \eA{BITS} have become
popular, producing human-readable \eA{PDF}s through techniques
like \eA{XSL-FO} while also enabling natural-language
content to be analyzed via generic \eA{XML} protocols, such as
\eASomeLower{XQuery} and \eA{XQSE} (\eASomeLower{XQuery} Scripting Extentions). \lJSON{}-based
formats are also used, like the \eA{JSON}-\eA{S2ORC} protocol adopted
by \CORDOneNine{} (mentioned earlier). Specifications such as the
Text Encoding Initiative (\eA{TEI}) may employ a generic
representation for convenience (e.g., \eA{TEI}-\eA{XML}) but they are
intrinsically a special-purpose data format optimized
for natural language (for instance, \eA{TEI}, established
in \SAyear{1987}, predates \eA{XML} itself). In a similar vein,
special markup languages like \eA{TAGML} (the Text-as-Graph
Markup Language) \mdash{} which are structurally
distinct from straightforward tag hierarchies (and therefore
not just a realization or dialect of \eA{XML}) have been proposed as more faithful
to the inner structure of text content than generic
formats (\eA{XML}, \eA{JSON}, etc.).\docFootnote{For \eA{TAGML}, see
\citeLabel{ElliBleekerEtAl} or \citeLabel{ElliBleekerEtAlTEI}. This
is one of the more recent proposals for markup languages
that incorporate \eA{SGML}-style features such as
concurrent/overlapping markup, and on theoretical grounds
might be considered a better conceptual match to
natural-language text than \eA{XML}. Of course,
although \eA{XML} was originally conceived as a more
practical and standardized version of \eA{SGML},
not all \eA{SGML} features were preserved, which
to some extent limited \eA{XML}'s
expressiveness.}
}% end paragraph 

\p.1{%
Debating the merits of, say, \eA{TAGML} versus \eA{JATS}/\eA{BITS} \mdash{} or
alternative \eA{XML} languages like \eASomeLower{BioC}, developed for
PubMed Central \citeLabel{DonaldCComeauEtAl} \mdash{} is outside the scope of this paper.
...
affect \textit{algorithms} that target the relevant
information. Any encoded data structure
is conceptually a \textit{space} in the sense
that we can use some terms of discrete topology
therein: we have a notion of \q{movement} within
the space; some parts are closer to each other
than others; we can consider indivisible
elements of the structure and picture them
as a \q{foreground} or \q{cursor} that
...
graphs: for instance, \q{property hypergraphs}
have nodes inside other nodes, plus
key/value attribute lists on nodes and/or edges.\docFootnote{For property hypergraphs, see \citeLabel{ArunaBansal}.
...
hyperedged contain other edges); the
...
features \citeLocator{NCDataIntegration}(pp.x--x).
...
concept of \q{topomorphic regimes}.}
}% end paragraph 
...
define a \q{structure} to begin with.
For example, an important contrast between \eA{XML} and \eA{TAGML} is that
the latter supports concurrent/overlapping markup,
which can only be achieved in the former indirectly
(e.g., via empty \q{milestone} nodes). Endorsing one
approach over the other is a rather abstract question.
A more concrete assessment can consider actual processing
tasks and address whether algorithms that \q{quantify}
over concurrent markup structures, or conversely
purely hierarchical ones, are easer to implement/test/maintain.
Assuming that most algorithms navigate around a structure
by tracking a \q{foreground} node \mdash{} some kind of visitor
pattern where one node at a time is the one visited
\mdash{} then \eA{TAGML}-style overlap complicates the decision-space
for node-to-node transitions. For instance, an element
that begins in the \textit{interior} of some quasi-parent node,
but \textit{ends} properly after the latter, does not have a
single unambiguous parent. After completing whatever
processing is needed for an element in this case, would one
more profitably revert to the \q{quasi}-parent in scope
at the \textit{beginning} of the element or, rather, at its end?
}% end paragraph 

\p.1{%
Moreover, \eA{TAGML} recognizes two distinct forms of de-facto
parent/child relations, namely \q{containment} versus
\q{domination}. In the latter case, one node has a
logical or semantic nesting inside another; the parent
forms a context which is semantically intrinsic to the
child (e.g., a \q{last-name} field in an overarching
\q{proper-name} tag). In \textit{containment}, on the other
...
in a single \eA{PDF} page, but paragraphs are not intrinsically
...
\eA{XML} has a single parent/child relationship. The more
complex \eA{TAGML} architecture may or may not make
...
in how code targeting \eA{TAGML} has to keep track of a
more complex space of parent-like internode connections.
}% end paragraph 

\p.1{%
Conversely, \eA{XML} algorithms would presumably be more
...
against a \eA{DOM} hierarchy is not a seamless translation
of node-overlap semantics to a non-concurrent
environment.\docFootnote{Milestone tags, in their
structural (emulative) purpose, are not
intrinsic parts of tools such as \eASomeLower{XPath}
and \eASomeLower{XQuery} \mdash{} apart from their basic status as zero-width
elements, which does not by itself convey their special
role.} If an \eA{XML} file does
indeed use milestones to emulate overlapping markup,
then the presence of landmark tags would have to be
tracked by \eA{XML} processors as state data or
a parallel hierarchy separate from the \eA{XML} structure
...
\textit{generic} formats like \eA{XML} or \eA{TAGML} skirts the
key point that here we are primarily concerned with
representing of \textit{text} content, not generic
...
\p.1{%
Insofar as \eA{XML}, \eA{TAGML}, \eA{JSON}, \eA{RDF}, and so forth are \textit{structurally}
different, such variation may more properly be
described as \q{meta}-structural, or concerning those
parameters that articulate the individual properties of an
\eA{XML} (etc.) structure in general. Any given instance
of \eA{XML} (etc.) structure takes its identity from the
full set of details endemic to the \eA{XML} format: element
names, attribute keys and values, text content (if any),
and nodes' parent/child and sibling relationship.
The parameters constituting a \eA{TAGML} instance are
...
difficult to compare \eA{XML} and \eA{TAGML} structure
\textit{instances} because what it means to be a structure
in the former case is not quite the same as in the latter.
We can, however, compare \q{meta} structures in that
we can present the suite of parameters through which
any single \eA{XML} instance is individuated, and by comparison
the analogous parameters constituting the \eA{TAGML} architecture.
Similar comparisons can be made \visavis{} \eA{JSON} and
\eA{RDF}-like formats (e.g., text encoding via property-hypergraphs).
}% end paragraph 

\p.1{%
For purposes of discussion, I will use terminology here to the
effect that different markup/representation formats
evince different \q{topomorphic regimes}, where \q{topomorphic}
literally means \q{shape of place}. This term,
...
to \q{visitor patterns} and variegated constructions
...
data is encoded via an aggregate of \q{sites} with
specific navigational possibilities from one site
to another. A topomorphic \textit{algorithm} is one
that extracts information by navigating to a site
of interest, being able to obstain specific data points
in the context of any one foreground site \mdash{} e.g., the
text content and attributes values (per key) of an
\eA{XML} element \mdash{} before traversing to a different site.
The \q{neighborhood} of a site groups all data that
can be extracted directly as well as nearby sites
accessible via a single transition-step. For
instance, the neighborhood of an \eA{XML} node
...
parent-ancestry.\docFootnote{An algorithm might only
ever need to visit child nodes in sibling order.
However, at least some \eA{XML} processors
present the option of moving to any
child node via a numeric index}
The neighborhood of a \eA{TAGML} node is similar but
more complex due to \eA{TAGML}'s looser attribute and
containment protocols. The neighborhood of an
\eA{RDF}-style site \mdash{} for instance, a hypernode
in a property hypergraph \mdash{} would be
...
\eA{JSON} \q{node} would be a tuple of values
(for an array), or key/value pairs (for an object),
or a single scalar value, depending on where the given
node was positioned in a \eA{JSON} hierarchy.
}% end paragraph 

\p.1{%
When text content is encoded via \eA{XML}, \eA{TAGML}, \eA{JSON}, etc.,
it takes on the \q{topomorphic} properties of the
...
elements in an \eA{XML} tree, assuming that paragraphs
...
how it is \q{serialized} via markup.
}% end paragraph 


\s|2|{Examples of Analyses Requiring Sentence Objects}

\p.1{%
The following cases may help to clarify why markup formats
\mdash{} when they are used not just for the interchange of text or metadata, but
as intrinsic components of publishing workflows \mdash{} can be
...
memory is more efficient than traversing \eA{DOM} nodes
and hierarchies. As such, a framework such as a
\q{Sentence Object Model} \mdash{}
where sentences and other discursive units are already
converted to code objects \mdash{} is more convenient
than (for example) \eA{XML} files where nodes must be
mapped to objects as a preliminary step.

\begin{description}


\descItem{Categorizing Inner Documents via Annotations}\hback{5pt} You are working with a
large manuscript assembled from many smaller documents. Their authors
have contributed \eA{PDF} comment boxes associated with specific sentences,
...
or utilized to assemble glossaries, enhanced \eASomeLower{ToC} pages,
web navigators, and so forth.

\pseudoIndent{} To complete this task it is necessary to execute procedures that
can obtain the content of all \eA{PDF} annotations, indexed by
...
up an \eS{std::map} style data structure whose keys
...
as necessary). These coding requirements \mdash{} working with
object-based data structures and local filesystems \mdash{} call
for programming in full-fledged languages such as \eA{C++} rather
than queries or \eA{XML} transforms.

\pseudoIndent{} As a related use case, consider how comment boxes
...
encouraged --- either directly through input sources or via \eA{PDF}
...
\q{Keynesian economics, and macroeconomic theory} and etc.

\descItem{Extracting Linguistics Annotations} You are working with a
...
as a basis for subsequent analysis. Such samples \mdash{} which
...
\mdash{} are commonly used in this discipline and typeset in
...
\pseudoIndent{} Because this mode of presentation is so common,
...
syntactic or semantic themes, particularly if each sample
...
common format (e.g., distinct \eA{XML} tags) to notate the examples;
but the full extracting and database persistence logic
would need specialized programming.

\descItemBlank{} \descMLabel{Embedding a Publication Viewer in a Chemistry Application}  You are
...
(e.g., special \eA{XML} tags) and have also added informative
suggestions via \eA{PDF} comments. Your goal is to find all
chemical terms/formulae which can be associated with
molecular-structure files in formats such as \eA{PDB}
(Protein Data Bank), \eA{MOL}, \eA{CML} (Chemical Markup
...
\pseudoIndent{} For this kind of context, special programming is needed to extract
textual data about chemical names and formulae,
and potentially to search for molecular files via science
\eA{API}s. Furthermore \mdash{} in the hopes of best leveraging \eA{3D} visualization
capabilities \mdash{} the \eA{PDF} viewer has to interoperate
with science applications, either through a common message-passing
protocol or by embedding the entire \eA{PDF} code base itself as a
plugin or extension. Ideally, one or more molecular visualization
applications themselves can be modified to work directly
with these special \eA{PDF} components, because \mdash{} once
\ThreeD{} files are loaded into the software \mdash{} the
...
\pseudoIndent{} Along similar lines, suppose a chemistry paper
includes various terms for named compounds, amino
acids, and so on (just considered in the molecular-visualization
context). With these specific technical entities, special-purpose
supplemental data and annotations can be relevant, such as
\eA{PDB} assession numbers, launch-requests for external
visualization tools, transcriptions between names and
formulae, etc. It would be reasonable to include access to
such information among context-menus activated both in the
\eA{PDF} window areas proximate to names/formulas in text
...
\mdash{} or at least hooks into points where menu options
are populated \mdash{} will ideally be synchronized
among different parts of a Research Object, including
text (e.g., \eA{PDF}) viewers.


\descItem{Papers with Geospatial Datasets}   Suppose a manus\-cript takes a place among a
Research Object with geospatial data, with
\eA{GIS} \q{attribute layers} encoded via columns
in the so-called \eA{XYZ} format (which is an alternative
to latitude/longitude more directly usable by
...
sets. Meanwhile, included code \mdash{} if it does
indeed seek to make the raw files usable for
analysis \mdash{} will almost certainly include
procedures for \eA{XYZ} to \latlon{} conversion,
...
\pseudoIndent{} This and the prior example might even be combined. Data sets
addressing environmental contamination (Such as the \eA{EPA}'s
Toxic Release Inventory, mentioned above in the context of
\eA{CSV} files) include both \eA{GIS} and Cheminformatics
objects. Harful substances can be detected
...
inolves. The former lends itself to digital maps;
the latter to molecular visualization
(which can help explain what makes a substance
dangerous to people or ecosystems). Class types
and \eA{GUI}s for both of these genres would
...
of the document-prep workflow that stands apart from
...
\mdash{} requirements that could well apply to
...
domain-specific \eA{XML} tags, but actually implementing
the cross-component interop needed to leverage this
such almost certainly requires programming over
...
text content (including objects modeleing rhetorical/discourse
...
\s|2|{Sentences are the Preeminent Units of\\Discourse}

\p.1{%
I will assume that the basic unit of intrinsic
text representation is a single sentence \mdash{} even
if this is not how most encoding standards
are designed. Among common formats, only \eASomeLower{BioC} and
\eA{TEI} appear to employ distinct sentence tags, and
even here these are optional \mdash{} thus, documents
in those formats may
have \eS{sentence} or \eS{s} elements (respectively) but there is no
...
subtle, because \textit{usually} sentences follow one another
...
sense \q{nested inside} the sentence where the footnote
...
\q{children} of the sentence that introduces it
(which in turn, potentially, ends with a colon \mdash{} or
no punctuation at all \mdash{} rather than the usual
period or question/exclamation mark). In short, sentences
are not \textit{directly} nested in others, but they can
...
inform how we model text content as a \q{space} to
...
phrase is found (perhaps augmented with \eA{PDF} page coordinates
for the sentence as a whole or the page-area spanned just
by the matched text \mdash{} which is how
results are typically presented by \eA{PDF} libraries' own search routines
\mdash{} assuming \eA{PDF} coordinate data is available).\docFootnote{The page-view rendering engine would then take
area coordinates and apply styling \mdash{} usually
background shading \mdash{} to visually mark
off search matches. The complications arise when
we intend to override the internal \eA{PDF} search
algorithms to match against encoded text instead
(given the problems of \eA{PDF} queries from
...
In special cases, however, \eA{PDF} page coordinates
of preselected items of interest (keywords, index entries,
etc.) can be preserved via mechanisms such as
\LaTeX{} \eS{pgfsavepos}.} In the absence
of any restrictions, every sentence in a document should
thus be checked, with potential branching to account
for \q{nesting} relationships (as with footnotes and block quotes, potentially)
as well as divergent possibilities for \q{next sibling} (consider a footnote
marked just after the end of one sentence: we can consider the discourse
as continuing on two tracks, one within the footnote and one the main text.
}% end paragraph 
...
conversely, perhaps they \textit{only} want to search within
footnote, quotes, bulleted content, etc. From a topomorphic
perspective, the relevant details here are not so much
the explicit \q{tags} or other markup via which special
...
of \textit{being} a footnote, block quote, inline quote, citation,
etc., is a \textit{characteristic} of text subsumed under the
relevant element, and users restricting query scope
should be understood as narrowing focus to content
with one or more specific \textit{facets} (not specific
\textit{tags} per se). This can apply to
...
and remembers certain words that were present and also
...
For instance, \eA{JATS}/\eA{BITS} has tags specific to personal
names, and their presence in article text allows
search tools to disambiguate proper names from their
homonyms (\q{Robin Hunter}, an actor, might also be
...
chemical formulae, phonemic transcriptions (for linguistics
...
can be reported as a two-byte \eA{UTF-16} code point.
...
\p.1{%
Such details of \textit{internal} representation, furthermore,
are distinct from user functionality like copy-paste
or \eA{PDF} annotations. For example, consider requests
...
\mdash{} \LaTeX{}, MS Word, email, a note-taking program, etc.
With no separate specification, \q{exported} content
for things like copy/paste should be as simple as
possible,\docFootnote{There is no guarantee that the program wherever
the copied content is pasted will understand anything
other than alphanumerics and basic punction}perhaps
even restricted to \eASomeLower{Latin1} (so
that \q{curly} single and double quotes, say, are
...
in double quotes. Quite plausibly the \q{input} to
that query could have either the straight/\eA{ASCII} form
...
the encoded text elements might not use quote delimeters at
...
might be requested in a variety of formats (\eASomeLower{Latin1}, \eA{UTF-16},
\eA{HTML}, etc.) such that how text is conveyed, in a variety of
potential reporting protocol, becomes data for each
...
A \eA{VM} could be engineered such that calls to a few general-purpose
\eA{C++} methods are reified as built-in bytecode instructions,
but such calls would need to be preceded by steps
to initialize all data relevant for the \eA{C++} evaluative procedures.
}% end paragraph 

\p.1{%
Meanwhile, a text-query \eA{VM} might have opcodes specifically
focused on navigation between/within sentence objects and
related text elements. Defining a rigorous \q{topomorphic}
model of written discoure's structures is a nontrivial
...
for a \q{topomorphic regime} supporting traversal
across text documents, with a detailed model of
what constitutes a single discourse \q{site},
...
\p.1{%
In a Research Object context, a \eA{VM} could be applied both for
...
written discourse, the \eA{VM}'s operations might also navigate
through various raw data formats, depending on how the
Research Object is encoded. \lVM{}s, that is, may need to be
adaptable for multiple kinds of \q{topomorphic regimes}.
...
Research Object \eA{VM}s in more general data-encoding contexts.
}% end paragraph 


\s|1|{Building a Self-Contained Compiler and Virtual Machine Runtime}

\p.1{%
For reasons already discussed, the kind of \eA{VM} relevant for this
...
to ensure that a stack sufficient to support (at least modest)
script and query-language capabilities can be developed
in an accordingly standalone fashion, with limited
external dependencies, included compilers for front-end
...
and does not require a user to first have something like
\eA{LLVM} or \eASomeLower{libClang} on hand. Two examples that I
am familiar with, and which might be relevant as case-studies,
are \eA{ECL} (Embeddable Common Lisp)
and \eASomeLower{AngelScript} \citeLabel{SimplexNumericaManual}, \citeLabel{HarijsGrinbergs},
both of which seem relatively easy to build from source. These
languages still are slightly different from what I envision
here, partly because \mdash{} despite being well-designed from the
...
execute scripts, with compilation and bytecode as implementation
needed) are comparatively straightforward to compile \mdash{} these
components are not primarily intended as providing
scripting/query layers for other code which is \textit{also}
distributed from source. For whatever reason,
almost surely \eA{ECL} and \eASomeLower{AngelScript} (and
other lesser-known languages with similar benefits) are not used very
...
\eA{VM} I consider here might well have special instructions
and/or front-end syntax rules that are unique to
the specific research being published, such that
front-end parsers, \eA{IR} and bytecode formats, and native-function
interop are designed to be extensible in a relatively
straightforward manner. This is admittedly a speculative
issue without a compiler and \eA{VM} tested in real-world context,
but at least by \textit{intent} the concepts I propose
here yield architecture and paradigms that offer more
accessible \q{hooks} for customization in contrast to
the internal code for languages like \eA{ECL} and \eASomeLower{AngelScript}.
}% end paragraph 

\p.1{%
In this discussion I will assume that the language and
any components it interacts with are written in \eA{C++}.
This is partly because the prototype compiler and
\eA{VM} I will occasionally refer to are written in \eA{C++},
and partly because I assume Executable Research Objects
will in general include certain \eA{GUI} components
(potentially including \eA{PDF} viewers themselves) and the
most straightforward path to effective cross-platform
\eA{GUI} applications is via \eA{C++}, particularly in conjunction
with the \eASomeLower{Qt} libraries. At the same time, as far as I am
aware, very little of the implementation details
I discuss involve techniques specific to \eA{C++}, so it
is entirely possible that similar \eA{VM}s, potentially
executing the exact same bytecode, could be programmed
in other languages (\eASomeLower{Java}, \eASomeLower{JavaScript}, etc.) \mdash{}
...
where \textit{both} an internal scripting and/or query language
and \textit{also} major \eA{GUI} and analytic components
are coded in \eA{C++} and distributed within the data set
...
for their own data publications. Nonetheless, if
each Research Object has its own specialized
\eA{C++} code, it can be unclear what role a \eA{VM} would play.
This is not analogous to large-scale scientific
applications that that embeds a scripting interpreter
for customizations, like \eASomeLower{Python} in the context of
\eASomeLower{ParaView} or \eASomeLower{ArcGIS}, \eASomeLower{JavaScript} \visavis{} \eASomeLower{OpenCV.js},
or \eA{C++} itself (as an interpreted language) in \eA{CERN}-\eA{ROOT}.
Clearly the entirety of \eASomeLower{ParaView}, \eASomeLower{ArcGIS}, \eASomeLower{OpenCV},
or \eA{ROOT} (not to mention \eA{LLVM} and \eASomeLower{Clang} for the latter)
and all their dependencies cannot feasbily be included
...
\textit{without} needing to install \eASomeLower{OpenCV}, or for
scientists to assess multidimensional data spaces
without having \eASomeLower{ParaView} (or comparable visualization
software) on their computer. This can be achieved
...
larger platforms (\eASomeLower{ParaView}, \eA{ROOT}, etc.).
}% end paragraph 

\p.1{%
But with that said, having to micro-customize
scientific \eA{C++} code can still add extra development
time to a data-publishing project. Again, I am
envisioning data sets including \eA{C++} classes that
are derived, at least in part, from libraries
intended to be used for many different projects.
Research Objects should avoid \textit{depending} on
external software such as \eASomeLower{ParaView} (though
\textit{facilitating} its use is fine), and this
may be achived by leveraging simpler components
for tasks such as interactive \eA{2D} visualization
(based on \eS{QtCharts} and/or \eS{QGraphicsView}, for instance).
Such components may take the form of a class or \q{widget}
library made available to users by publishers, book series,
journals, grid maintainers, etc. (similar comments apply
to digital cartography \eA{GUI}s in lieu of \eASomeLower{ArcGIS}, image graphics
to interactively demonstrate processing steps
originarily executed via \eASomeLower{OpenCV}, and so forth, just
...
implemented simply by modifying the relevant \eA{C++} code.
...
\p.1{%
I therefore see numerous use cases for a \eA{VM} to buttress
scripting or query languages that express
customizations that would be more complex
to achieve via manually editing \eA{C++} code itself.
At the same time, computationally intensive routines
\mdash{} or simply ones that for stylistic reasons are
better suited for it \mdash{} might still be
coded (specifically for the data set) in \eA{C++}.
One consequence is that interfacing with
\eA{C++} should be easy. Making calls to \eA{C++} procedures
(and marshaling input/return arguments) should
require less effort that evident in languages
like \eASomeLower{Python}, \eASomeLower{JavaScript}, or \eASomeLower{Haskell}, which
serve more as generic languages in their own right.
}% end paragraph 

\p.1{%
%
Conceptually, a \q{foreign-function interface} (\eA{FFI}) to
\eA{C++} procedures is relatively straightforward. The called
...
i.e., supply the left side of a \eI{CLASS::*METHOD-PTR} construction).
Methods and procedures can be called, in general, through
type-agnostic function pointers so long as the byte-lengths
of all arguments match what the procedure expects.\docFootnote{A terminological
note on words such as \q{procedure} and \q{function}. In most
contexts I prefer the former, because there is less confusion
with the more mathematical sense of \q{function} as a
value-to-value mapping. Conversely, when discussion language
implementation and specification for the likes of \eA{C++},
\q{function} is the more common term. I believe a useful
contrast is as follows: \q{procedure} can typically be
...
the composition \fOFg{} means to call \eH{g} first,
then \eH{f} with its result. Here \q{composition}
acts like a binary operator on two function values.
For languages that support similar notation,
certain function-values could be expressed
...
complicate the picture a little, but still for \textit{most}
\eA{C++} procedures it is possible to compress all data needed
to call the function indirectly (through a pointer) to a
single two- or four-byte integer, which (for the lack of an
agreed-upon term) I will call a \q{micro-signature}.
\q{Registering} a \eA{C++} procedure with a \eA{VM}/script engine
...
\eA{FFI} step is noticeably more complex in \eA{ECL} and \eASomeLower{AngelScript}).
The code libary published along with this paper
demonstrates how micro-signatures map to
steps for arriving at the action function-call,
via a series of switch blocks \mdash{} in this context
\eASomeLower{AngelScript} defers to inline assembly code, and
other languages fall back on \q{smoke} bindings
or other pre-processors, or else load in a large
dependency such as \eH{libClang}. So the approach described
here, by comparison, is more aligned with the
idea of a compiler and \eA{VM} being both standalone and
...
code \q{works} with the switch-branching mechanism
(this is a necessary compromise, because too many
switch cases can make a binary uncompilable).
The goal however is that \textit{most} \eA{C++} procedures that a
data set would want to expose to script engines have signatures
that reduce to a supported microsignature code.
For those that do not \mdash{} considering that the
Research Object is expected to have some \eA{C++} code of its own
\mdash{} it should not be difficult to implement wrapper
...
those functons whose codes do not fall within any
of the supported equivalence classes.
}% end paragraph 

\p.1{%
Another idiosyncratic of the demo \eA{VM} is what the code
calls \q{channels}. The goal here is to design an
abstract/ generic model that
could be adopted to different calling conventions and
\eA{ABI}s relevant to the \eA{VM} environment, both the
host language (which, in the general case, is not
necessarily \eA{C++}) and calls to procedures
implemented via the \eA{VM} itself (i.e., as opcode/argument
...
\eA{VM} are expected to be relatively small and have
...
named for result values returned from \textit{constructors}
...
technically deques (double-ended queues) where values
(called \q{carriers}) may be pushed in front or from behind.
The operation to do so is an execution point that
can involve \q{effects} such as a value being read
...
points in particular, and potential install runtime
handlers or monitors. It's worth noting that the
\eA{VM} \q{instructions} are themselves actually calls
to ordinary \eA{C++} methods, which can be extended, reimplemented,
or debugged (or \q{breakpointed}) like any other
\eA{C++} procedure. This gives the \eA{VM} greater operational transparency
...
\s|2|{Proof-Scope and Contract Overloads}
...
algorithm preconditions, including \q{assert} statements,
...
Design by Contract (\eASomeLower{DbC}). In this section I will
consider how \eA{VM}s can facilitate scripting languages
that incorporate these features.
}% end paragraph 

\p.1{%
Consider \eASomeLower{DbC} in particular. The fundamental insight
...
ae extensional properties of types themselves (for
instance, a signed one-byte integer can never have
values above \SAnmbr{127} or below \SAnmbr{-128}). However,
many properties of interest are true only of
\textit{some} instances for a given type. For example,
in most file-system libraries there is no separate
...
whose role is to initialize a \q{double-associative}
array or \q{reversible map}; that is, a tuple of pairs that uniquely map
...
second to the value type; moreover, for invertability,
the two lists must be the same size. An implementation
may thereby assume this as a precondition.
}% end paragraph 
...
how to enfore this. Such a procedure could simply
...
\p.1{%
Explicitly checking preconditions via if/then
tests is better than blind faith, all else being
equal, but that convention does violate
\q{separation of concerns}: arguably, checking
...
into a different procedure. In other words, some
distinct code should perform the \q{gatekeeping}
role of rejecting contract-violating inputs,
and the visible separation of that logic from
the main procedure-body can help one
understand and maintain the code.
}% end paragraph 

\p.1{%
Steps to guarantee that contract-annotated procedures
are \textit{only} called with preconditions duly satisfied
can be taken either at runtime or compile time (or both).
In general, compilers can assess whether it is \textit{possible}
for a contract to be violated at a given call-site.
If this \textit{is} possible, then the compiler can insert
an extra runtime call that serves as a \q{gatekeeping}
check. If it is evident that a contract necessarily
\textit{cannot} be upheld, then this could be a compiler
error immediately. Otherwise, if the compiler
can ensure that for all execution branches, external states,
or value-combinations a contract is \textit{guaranteed}, then
...
similar. For example, recent \eA{C++} contract specifications
imply that the typical results of a contract violation
are either calls to a \q{handler} (which might implement
some alternative sequence that avoid depending on the precondition
givens, or potentially throw an exception) or immediate
program-termination \citeLabel{ContractsforC++}. Structurally this is not too far off
from the handler being a \textit{de facto} \q{catch} block that
could either rescue or propagate an exception. As far as
I can tell, these semantics are similar to those
employed by most \eASomeLower{DbC} supporting languages.
}% end paragraph 

\p.1{%
This all seems wrong. I believe a more intuitive
framework would employ what might be called \q{overload by contract},
...
\eA{VM} use-case I wish to examine here is to support \q{Overload by Contract}
(\eASomeLower{ObC}) as one version of \eASomeLower{DbC}.
}% end paragraph 
...
know that it is \textit{not} satisfied; or, that status might still be
...
(or vice-versa).\docFootnote{For the map to be invertible, these cannot just
be default \textit{constructed}} A \eA{VM} compiler and runtime should not attept to
...
that addresses violations. By analogy, when inherting from an
...
\q{overload} in response. In a same-size list case, logic
...
function wuold then be analogous to an entry-point for
...
evaluate equal (as in \eH{x.size() == y.size()} or similar) and no
...
to be explicitly notated in code \mdash{} both at call-sites and within
signatures \mdash{} rather than enforced by invisible machinery behind the scenes.
...
by one and thereby changes its types).
}% end paragraph 

\p.1{%
In practice, then, we should not assume even that the compiler would recognize an
if-expression like \eH{x.size() == y.size()} as testing the exact same state of affairs
...
context, one way to achieve this is via what might be termed a \q{reified boolean}.
In this sense, the result of \eH{x.size() == y.size()} is not just a boolean
true or fale, but a more substantial object that \textit{evaluates} to boolean
in the first clause of the \eH{if}/\eH{then} expression, but also persists as an
...
to evaluate not only to a boolean return value but a \q{reified} object.
}% end paragraph 

\p.1{%
When an \eH{if}/\eH{then} construction involves a \textit{reified} boolean I assume
this object remains \q{in scope} for the associated code blocks; and
that similar branching or loop formations could have similar
conventions. Conceptually, \eH{if-then-else}, \eH{do-while}, etc., don't
necessarily have to be special constructions in the language;
they could possibly be expressed as ordinary procedures, taking
code blocks as arguments. In this case, the language could
have a mechanism such that procedures which \textit{call} code blocks
...
\p.1{%
The notion of \q{channels} also comes into play.
Conceptually, channels capture data from a call-site scope. If a procedure
declares its reliance on or assumption of a contract \mdash{} e.g.,
two of its parameters having equal list-size \mdash{} then in the
current context I interpret this as requesting a \q{proof}
object to be passed in through some specific kind of contract-verification
channel. Obviously that channel would not \q{look} in code
...
to popuate it with objects in the current lexical scope.
A reified boolean such as one passed into the scope
by an \eH{if}/\eH{then} test could serve this purpose.
...
validity, which depends on both the proof \q{type} and
...
the list paired with it for the double-array. In
...
method \textit{might} change a size, it would do so
...
obligations that need to be fulfilled.
}% end paragraph 

\p.1{%
There are also cases where a codewriter might be confident
that the kind of proof embodied by a reified boolean
applies even in the absense of a test like
\eH{x.size() == y.size()}. If the second list
is obtained from the first by something like an
\eH{std::transform}, they will be guaranteed to have the
...
current scope (if desired, the expression which achieves
this could, under debug modes, also serve behaviorally
as an assertion).
}% end paragraph 

\p.1{%
This arrangement \mdash{} coupled with an Overload by Contract
convention that procedures requiring that contracts
are overload in several versions (at least, those for
pass, fail, and testing of a relevant contract) \mdash{} may
not be the most powerful engineering of a \eASomeLower{DbC} system,
but I believe it makes the compiler's role straightforward.
Correspondingly, it is clear how the compiler should issue the relevant
\eA{VM} instructions. I believe this serves as an example of
how channel-based calling conventions provide a roadmap
to guide compilation steps that might be convoluted otherwise.
}% end paragraph 


\s|2|{Variable Lifetimes and Memory\\Management}
...
\p.1{%
Consider return and/or initialization options in \eA{C++}, on the
assumption that we are not simply returning by value directly
due to size concerns. One possibility is to allocate a pointer
...
that can be given a default or preliminary initialization
and then have their \q{real} values (of internal data fields)
...
memory-size and the callee to initialize \textit{that} via
placement new. The memory itself could then come either
from the stack or the heap, whereas there is no way
for a callee to allocate values on the \textit{caller's} stack.
Unfortunately, there is no way to implement this kind
of arrangement in
ordinary-looking \eA{C++} (this mechanism is similar
to how compilers implement Return Value Optimization).\docFootnote{For Return Value Optimization (\eA{RVO}) see \citeLabel{SeanMurphy} or \citeLabel{AntonZhilin}.
Readers might be interested in an overview of
user-visible \eA{RVO}-related techniques that I present as
a \eA{C++} code excerpt at \hlinkOne{https://cppinsights.io/s/cac923c9}
which also addresses lifetime issues as discussed next.}
}% end paragraph 

\p.1{%
Which alernative is most convenient from a caller's point
of view? Pass-by-reference is a common \eA{C++} pattern:
...
here \mdash{} apart from cases of non-default-constructible
types already mentioned \mdash{} is that there is a visible
separation between the point of declaration (which is
also a minimal initialization) and the \q{de facto}
intialization effectuated by the called procedure.
}% end paragraph 
...
Moreover, sometimes the callee \textit{wants} a
...
to work on a value/object \textit{either} passed
...
which uses memory buffers behind-the-scene
...
can serve one of several purposes:

\begin{enumerate}

\enumsItem{}   The primary purpose of the callee is to initialize or modify
that object
\enumsItem{}   The object serves mostly to provide information the callee
...
\enumsItem{}   Modifying the object is a primary role of the callee,
...
is \textit{guaranteed} to modify (as when a default-initialized
...
others may be secondary and just represent a
...
\textit{primary} purpose of the procedure are those
effects, and that the return value is mostly a sanity
check. Conversely, it may be that the primary purpose
...
to what purposes or effects are \q{primary} and \q{secondary}.
Nonetheless, programmers and maintainers will reason
...
these issues, more so than in \eA{C++}, for example.
For instance, with pass-by-reference there is no
visible mechanism to distinguish object that
will \textit{necessarily} be modified, or will be
so assuming the callee returns normally,
or instead only \textit{might} be modified.
}% end paragraph 

\p.1{%
In \eA{C++}, pass-by-reference serves as a workaround
...
by referenec even if, behaviorally, the
role the callee vis-a-vis the object is
more like that object being \textit{returned by}
the callee.
}% end paragraph 

\p.1{%
A scripting language \mdash{} with more freedom to tweak its
syntax \mdash{} could do a better job of flagging its caller/callee
semantics. For example, assignments (in \eA{C++}, the left
side of a basic operator equals) may be
\textit{initializations} (for a variable that, beforehand,
was only declared) or else \q{re-initializations};
there is no syntactic difference. A variable declared
and then passed by reference will first be
\textit{uninitialized} (for built-in types) or \textit{default initialized}
...
versus pass-by-reference (or \eA{RVO})
one option would be what might be called \q{Janus}
(metaphorically \q{two-faced}) channels
that (unlike most) may be either input or output.
For sake of discussion, suppose we call a \q{Janus pointer}
to be an object that, depending on context, could
serve as a pointer, reference, or non-initialized memory buffer.
Here \q{Janus} is not some pointer typeclass but
instead a formation placed inside a Janus channel.
When assembling the \q{channel package} for a procedure-call,
the compiler can check whether an \textit{initialized object/value}
or else a \textit{memory buffer} has been pushed into the channel.
If so, that gets passed as \textit{input} to the callee (in the
latter case the callee is orchestrated to call placement
new prior to its main execution). If a Janus channel
...
\p.1{%
My overarching point is that, for procedures that employ
...
within the preamble portion of an \eH{if}/\eH{then} or \eH{for}-loop expression,
one obvious question is whether the object
is associated with the \textit{inner} code block or the surrounding scope
(and, hence, when its lifetime ends and destructors are
called, if relevant). In \eA{C++}, the specifics are forced by
syntax. I envision languages where (perhaps with inobstrusive
...
same otherwise. For example, \eH{for}-loops should
...
conditions, iterators, etc.).\docFootnote{A good introduction to \eA{C++} lifetime issues \mdash{}
and solutions via \q{lifetime annotations}
(often associated with the \eASomeLower{Rust} language)
\mdash{} is \citeLabel{MartinBraenne}. I believe some of these
...
code point (their \q{retirement} aligned
to their scope's end) or aligned with sibling variables.}
}% end paragraph 

\p.1{%
Procedures' \q{interface} is more than just an implementation detail,
moreover, because at least some routines within a code library
are \q{semantic} entities that should be searched and documented
...
\mdash{} and perhaps
an introduction or \q{manual} outlining its content.
...
\p.1{%
The analogous documentation for computer code would be serching
...
should be more fine-grained. For instance, detailes such as
...
\q{executable} resource(s) of an Executable Research Object
...
Because the \eA{VM} is designed to support formations
...
\s|2|{Dependent Types and Template\\Metaprogramming}
...
are lists of length \lLen{} is \q{dependent} in that
its specification depends on a \textit{value}
(by contrast to \textit{meta-types} that depend on a
\textit{type} to be specialized, as in
\eS{vector<T>} where \eS{T} could
be \eS{int}, \eS{double}, etc.).\docFootnote{For the general motivation and theory
of dependent types a good
source is \citeLabel{EdwinCBrady},
which looks particularly at
the \eASomeLower{Idris} language
\mdash{} one of just a small group
of relatively general-purpose
languages to support
them. An interesting look at potential
applications for languages in general
is \citeLabel{PottayilHarisankerMenonEtAl}.} If the aforementioned \lLen{} is a compile-time
constant, then the relevant type
may be called \textit{weakly} dependent.\docFootnote{Although \q{strong} and \q{weak} sometimes
appear in related contexts, my proposed
usage is not the same. But in the general
case \mdash{} considering just types themselves
\mdash{} a dependent type is one of a type
\textit{family} associated with some
other \q{index} type, such that
...
variable/symbol is itself obtaine
from some \textit{other} variable, then
the former also \textit{depends on} the
...
referred to as \q{strong} dependence;
these are usually the situations
motivating dependent type systems.
For example, \eA{C++} certainly allows
...
reversible map would involve a \textit{strongly}
...
\p.1{%
It is \textit{not} a matter of strong dependence
to construct a type whose instances are \textit{pairs of}
...
involving the \textit{procedure} being called (and so
implemented via contracts or other precondition enforcement).
}% end paragraph 

\p.1{%
Expressing relevant constraints via \q{just} the type
...
types and their extensions.\docFootnote{This is not
true of all types; one has no issue equating
the extension of a \eS{byte} type with unsigned
integers less than or equal
to \SAnmbr{255}.} The fact that a hypothetical
value \eH{v} would be one instance of a type \eH{t}
does not confirm that \eH{v} belongs to
\eH{t}'s \textit{practical} extension at any point
...
code the \textit{purpose} of a type is not
...
\p.1{%
In a language such as \eASomeLower{Idris}, where dependent
typing is built in intrinsically, types
become essentially \q{runtime} phenomena
...
\q{non-extensional} type theory. Once
...
say, \eH{y.size()}, but that's not doing
...
details like \eH{y}'s \q{granular} characterization
are not types per se, but rather
something like an annotation or
axiom asserted relative to \eH{y}.
}% end paragraph 

\p.1{%
None of which precludes developing something
like a dependent type system, but the
class of \q{dependent} types has to be
...
blocks, one for each \q{tapset} (we
might say, meaning \q{type-attribution
per symbol}). For purpose of discussion,
I propose the term \q{type \textit{facet}}
...
For instance, any \eS{float} (well, any rational/decimal) may
have fractional part zero, and so
be (losslessly) convertible to integers.
The fact that a \eS{float} \eH{f} could also
be \eS{int}, say, is a detail whose logical
form can be expressed via types.
This appears, indeed, to be a contract
or precondition, but a special kind.
Here one way of stating the contract
is that \eH{f} is \q{compatible} with
\eS{int} in some sense.
}% end paragraph 
...
to \q{constructors} (or factory functions \mdash{} specifically,
...
in a language like \eA{C++}). More precisely,
in a channel system we can define a constructor in
a general sense as any procedure with an output
channel marked for a \eI{constructor} role. Then a
\q{constructor contract} could be any contract
...
value is exactly \eH{n}. The test for a
constructor-contract in this case would
presumably just test an \eH{f}'s being at
least \eH{n} and less than \eH{n+1}.
Insofar as some \eH{f} passes the test,
then the hypothetical type can serve as a
\textit{facet} even if no actual constructor
...
without some bytecode block being duplicated).
}% end paragraph 

\p.1{%
In short, some form of dependent
types could be allowed if they may be
implemented more as value-annotations
consistent with this templating mechanisms
than as actual types attributed to their bearers.
Not only does this approach clarify
dependent-typing possibilities, it also
indicates how template metaprogramming
might be supported in a \eA{VM} context.
}% end paragraph 

\p.1{%
Of course, I have implicitly
made assumptions about the
\eA{VM} in question. Clearly the
\eA{VM} would work in general with
...
for \eA{VM}s in the Research Object
milieu relevant to this paper,
these are reasonable assumptions.
}% end paragraph 


\s|1|{Extensibility \Visavis{} Grammar and Intermediate Representation}

\p.1{%
Here I address some further details about how
such \eA{VM}s could employ robust type objects.
...
\s|2|{Decorated Types and Axiation}
...
to dimensional analysis and (for instance) \eA{SI} units
in a scientific context, but they also extend to areas
which do not obviously employ numbers measuring physical
magnitudes such as size, time, temperature, etc.
}% end paragraph 

\p.1{%
Consider the case of \eA{2D} graphics. There are many
\eA{C++} graphics and image libraries, and they are inconsistent
with respect to what the horizontal and vertical axes are
called, as well as which quantity comes first in a pair.
A \eASomeLower{Qt} rectangle, for example (e.g., in \eS{QGraphicsScene} windows)
...
width and height. \eS{QPoints} are integer pairs (\eS{QPointF} is floating-point instead)
in \eH{x}-then-\eH{y} order. Here the dimensions are conceptualized
as \eH{x} and \eH{y} axes; but in other contexts pixels are treated
rather as \q{cells} as if in a table, and coordinate-pairs
are in \eI{row}-\eI{then}-\eI{column} (i.e., \eI{vertical}-\eI{then}-\eI{horizontal}) order.
In the case of \eS{QRectangle}, one similarly must commit
to memory that the first scalar is \q{width} and the second \q{height}.
}% end paragraph 
...
\q{moving} a point some distance left/right then up/down
can be notated by convoluting the point with a
transform pair rather than disconnected scalars
(as with \eASomeLower{QTransform}, affine translations are one
...
merged into a single \q{size} parameter.
...
as \q{\eI{width-then-height}} \mdash{} a different type than
\q{\eI{x-then-y}}. More substantially, we can consider a
spectrum of distinct \eA{2D} coordinate types representing
how horizontal and vertical axes are conceptualized:
\eI{x-then-y}; \eI{row-then-column}; \eI{first-then-second} (as with
generic pairs); \eI{width-then-height}; or \eI{horizontal-then-vertical}
(plus inverses of all these). The strings
\q{\eH{s4wh}} or \q{\eH{u2cr}}, say, might encode signed 4-byte
\eI{width-then-height} pairs and unsigned 2-byte \eI{column-then-row} pairs,
respectively (\q{negative} width or height could cover
...
objets are compatible with the procedure. Alternatively,
...
matches for searches given such terms as \q{\eA{FPS}},
\q{kilohertz}, etc. Insofar as a code-domentation \q{concordance}
...
uses the term \q{carrier-handoff} to refer to
actions such as transfering a value from one package's
\q{return} channel to another's \q{lambda} channel
(which is the default for inputs). This would be a natural
place for runtime \q{hooks} confirming a range.
}% end paragraph 

\p.1{%
In brief, in a channel-oriented framework \textit{most} of a compiler's
...
inputs to another. The \eA{VM} instructions which are \textit{not} focused
...
\p.1{%
These comments apply mostly to \textit{semantic} customization. They pair
naturally with the issue of extending \textit{syntax}, which I discuss next.
}% end paragraph 


\s|2|{Language Grammar and Intermediate Representation}

\p.1{%
External tools like \eA{LEX} and \eA{YACC} make compilers less self-contained,
...
Several of these exist for \eA{C++}; within Research Objects employing
\eASomeLower{Qt} for cross-platform \eA{GUI}, the \eASomeLower{QRegularExpression} classes are adequate.
}% end paragraph 

\p.1{%
Meanwhile, most \eA{C++} regular expression libraries take ordinary
strings as initializers for regex patterns. This makes it possible
to add features to the basic regex language, so long as these
may be expressed via ordinary regex patterns that are computationally
generated.
}% end paragraph 
...
to a \q{slash-A}, i.e., \slashA{}, in Perl-compatible expressions). That is, no rule
will \q{skip over} characters, and rules will not in general shift the \q{current}
...
an anonymous procedure, like a \eA{C++} \q{lambda}) and the first matching
rule's callback is executed, preempting other potential
matches (since all patterns are anchored at the same place, there
is no need for a \q{ranking} or backtracking due to some matces being further
...
\p.1{%
Conceptually, the goal of an initial parsing run would be to construct
an Intermediate Representation or \q{parse graph}. Schematically, the
...
\p.1{%
For sake of discussion, then (this is the apprach taken by the demo code)
each rule's callback works primarily by emitting code for a kind
of preliminary or preparatory \eA{VM}. This is not the \eA{VM} whose
bytecode is the overall compilation target, but rather a kind of
miniature \eA{VM} whose operations are oriented either to
building up a parse graph or emitting instructions of the \q{real}
\eA{VM} (or both). In general, particular nodes of the parse graph
can be associated with lists of \eA{VM} statements, and the eventual
...
workflow might proceed by creating a \q{rough draft} of the
...
\eAAllLower{x86} registers). Both for the \q{preliminary} and \q{primary}
\eA{VM}s a single instruction can take either a single value
or no value at all. The instructions in general could
be interpreted by a \eA{C++} class, such that each instruction
maps onto a \eA{C++} method. For development and testing,
the \q{bytecode} files could actually be stored
...
one version of the \eA{VM} interpreter can be
...
\p.1{%
Given this setup, extending the \eA{VM} should be conceptually
...
handlers. New \q{preliminary} \eA{VM} instructions may
likewise be implemented similarly via methods in the
pre-\eA{VM} interpreter, and the rule-callbacks can
work by emitting instructions for the pre-\eA{VM} extensions.
Custom channels might be employed to implement
special-purpose calling conventions and \eA{ABI}s.
The kind of overall compiler/\eA{VM} stack outlined
here has, I believe, a relatively clear \q{roadmap} for where
...
\eASomeLower{Python} or \eASomeLower{JavaScript} (or even fairly \q{insular}
languages such as \eASomeLower{AngelScript} and \eA{ECL}).
}% end paragraph 


\s|2|{Script-Enabled Parsing for Deserialization}

\p.1{%
The pure-\eA{C++} parsing system described above could
be isolated from the rest of the workflow sketched
in this section. In so doing, one option is to
register a \eA{VM} interpreter with the grammar
...
via \eA{VM} bytecode, where the \eA{VM} in question is
...
Of course, if generic formats like \eA{XML} or
\eA{RDF} are employed for encoding, the \q{topomorphic
regime} is already implicit in the representation standard.
}% end paragraph 

\p.1{%
In any event, it makes sense for a \eA{VM} serving these
sorts of use cases to have built-in instructions pertinent
to topomorphic computing, i.e. to \q{visiting} and
\q{navigating between} sites (e.g., graph nodes). Perhaps
such generic instructions could call \eA{C++} methods overriden
for different kinds of input sources and their \q{regimes};
...
from associated publications, the \eA{VM} has the ability to
visit and traverse both raw data structures and textual/linguistic
content present in a Research Object. Both sorts of
materials are tractable to the \eA{VM} in a common search
and front-end protocol. In short, the tools for working
with raw data \mdash{} via a \eA{VM} implemented internally
in an Executable Research Object \mdash{} encompass
...
whereas the second was involved with \eA{VM} and
...
components' \q{public interface} stands in an environment
where synergy between code-related searching
and querying over text or data sets is one
design goal. The structure of precedures' input and
...
intended. Mergnig pointer-return, reference-initialization,
...
varible lifetimes through explicit
...
to ones envisaged here \mdash{} where raw
data files are paired with deserializer
code such that data sets' contents get
converted to runtime type-instances \mdash{}
...
capabilities \mdash{} plus potential
script or query languages that access
them \mdash{} in a unified manner. Within
