...
data. According to \eA{FAIR} (Findable,
...
with their own requisite functionality. \enlargethispage{6pt}This affects functionality
...
\p.1{%
Relatedly, researchers have explored  
text mining/Natural Language Processing tools to facilitate
...
As one example, the \CORDOneNine{} corpus was curated by the
...
\p.1{%
\enlargethispage{1em}External tools like \eA{LEX} and \eA{YACC} make compilers less self-contained,
...
Regular Expressions in a (one might say) \q{unfolding} grammar premised on
rules anchored to one \q{cursor} position \mdash{} which is continually
advanced forward \mdash{} have their own techniques, and one frequent issue is
...
\q{read} that content to handle
the \textit{current} rule.} Meanwhile, most \eA{C++} regular expression libraries take ordinary
