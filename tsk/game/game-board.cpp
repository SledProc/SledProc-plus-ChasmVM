...
// move_indicators();
 ...
 auto icons = [&icons_text, in_folder, &north_copier, &south_copier, this]()
 {
  ...
  QString icons_folder = in_folder + "/stone-icons";
  ...
  for(QString piece : pieces)
  {
   ...
   south_copier +=
     "\n<use id='%1' class='south-stone-icon_copied' "_qt.arg(south_id);

   south_copier += "xlink:href='#south-%2_proto'/>"_qt.arg(piece);

   north_copier += "\n<use id='%1' class='north-stone-icon_copied' "_qt.arg(north_id);
   ...
   driver_->register_south_stone_icon(south_file, south_id);
   driver_->register_north_stone_icon(north_file, north_id);
...
<a class='south-token_base south-token_in-play' id='%1'>
 <circle cx='0' cy='0' r='13' />
  ...
  <!-- stone icon representations -->
  %2
  <!-- end stone icon representations -->
  </g>
</a>
