...
protocol (\Scign{}-index) to standardize \Cpp{} 
...
\q{Executable} Research Objects (alternatively called 
\q{Research Compendia}), yielding a proposed component-oriented  
model that \PacTk{} calls \ScignTier{} (\Scign-indexed/text-integrated 
Executable Research components).  In component-oriented programming, 
...
we intend to annotate the relevant classes with \ScignTier{} metadata.}
...
\begin{figure}[h!]
\hspace{-45pt}\raisebox{-6pt}{\includegraphics[page=53, scale=.56, trim=0 11cm 0 0, clip]{UserManual.pdf}}%
\hspace{-18pt}\includegraphics[scale=1.1, trim=0 3cm 0 1cm, clip]{baseScreenshot.png}
...
code packages may be downloaded and run privately, 
...
(Assume that the paper includes frames from a simulation video and 
...
of publishers --- PLOS One in this case --- as scientific code).}
...
\CHASTE{} source code and filesystem layout.}
...
maps, which is often much different than the 
information's internal structure.  Web maps, lacking multi-window display, tend to have a similar problem.}
...
The display should be annotated as a multimedia resource, 
...
molecular visualization, biosimulations, etc.).}
...
viewpoint.  Individual forms are largely autonomous: each 
one's contents not intrinsically dependent on another's.  
...
\textit{de facto}.  Thus, \FAIR{} grids should certainly 
...
eliminate license fees (a considerable savings 
in hospitals with a large staff).  Among extant 
...
recognize that forms have use-cases even in the 
...
and become familiar with the window's \GUI{} controls.  
...
could be employed to track a few records for users 
...
could visit for tutorials and other exploratory contexts 
...
\begin{figure}[h!]
\hspace{18pt}\includegraphics[scale=0.5, trim=0 2mm 0 6mm, clip]{fin/sah.png}
