...
and Para Ids, and shows how the match appears in raw text 
...
intra-paragraph continuation text after block quotes or enumerations.  Special formats 
...
\textcolor{clrmcMid!20!gray}{$ii$}) for block quotes, 
and English letters (\textcolor{cdotchere}{a}, \textcolor{cdotchere}{b}, etc.) for 
intra-paragraph after block quotes or enumerations.  Special formats 
are also applied to the portion of paragraphs at the\\top 
of a page/column (near the bottom of the screenshot, 
these styles are visible in the continuation 
...
text width=5.84cm, inner sep=13pt, font={\fontsize{9}{9}\selectfont}] (ltext-outer)
 at (3.47, 6.5) {
This dialog window shows data for a single index-entry, including \makebox{headings}, 
entry/subentry 
relations, redirections (printed in an index as ``\textit{see}'' 
and ``\textit{see also}''), 
...
(s1) at (.52, 3.8) {{
This screenshot demonstrates an example of a dialog box that could be used to study the 
...
(when adding to the index database) or keyphrase 
(when searching).   
}
};
