...
in PDF view and raw text.  PacTk identifies the matching page\\numbers 
...
(s1) at (1.4, 5.1) {{Paragraph ids for this PacTk document 
...
text width=8.2cm, inner sep=13pt, font={\fontsize{9}{9}\selectfont}] (ltext-outer)
 at (1.33, 5) {Paragraph ids for this PacTk document 
...
fill = margin!10, inner sep=1pt] at (7.36, 12.1) (char)
{42};
...
\ifnum\value{page}=18{
\AtTextLowerLeft{\hspace{18pt}\raisebox{320pt}{
...
\node [draw=red!60!black, fill=blue, anchor = south west, rounded corners,
text width=15cm, inner sep=17pt, xshift=-8pt, yshift=-8pt, font={\fontsize{9}{9}\selectfont}] (ltext)
 at (2.4, 11.5) {{Converting PDF files to SVG pages (as mentioned above) allows each page to 
...
\node [draw=red!60!black, fill=white, anchor = south west, rounded corners,
text width=15cm, inner sep=13pt, font={\fontsize{9}{9}\selectfont}] (ltext-outer)
 at (2.33, 11.44) {Converting PDF files to SVG pages (as mentioned above) allows each page to 
have its own web address.  This enables publishers to organize pages 
into an online concordance indexed by topic or by annotated content.
};
...
\node [draw=red!60!black, fill=blue, anchor = south west, rounded corners,
text width=14.8cm, inner sep=17pt, xshift=-8pt, yshift=-8pt, font={\fontsize{9}{9}\selectfont}] (ltext)
(s1) at (3.34, 1.9) {{Two book excerpts showing inaccurate HTML generation 
from intermediate files whose primary purpose is to produce 
PDF documents (\tcircledb{1} and \tcircled{2}). 
The common problem visible in the two examples 
is that textual or character segments which should be treated as 
horizontal units (subject to glue-spacing algorithms 
vis-\`a-vis words to their left and right) are, incorrectly, isolated 
on their own line, as if they were \q{vertical mode} boxes instead.  
Such errors naturally disrupt the surrounding lines (ones that have normal 
content instead).  The proper way to address these issues is to 
specify project-specific HTML encoding rules for non-standard tag entities and/or 
sentence-level (or smaller) objects, rather than relying on 
relying on generic HTML generators.  The excerpt at the bottom of the 
screenshot shows a correct rendering of the left-hand passage
(\tcircledl{3}).}
};


\node [draw=red!60!black, fill=white, anchor = south west, rounded corners,
text width=14.7cm, inner sep=13pt, font={\fontsize{9}{9}\selectfont}] (ltext-outer)
 at (3.33, 1.8) {{Two book excerpts showing inaccurate HTML generation 
from intermediate files whose primary purpose is to produce 
PDF documents (\tcircledb{1} and \tcircled{2}).  
The common problem visible in the two examples 
is that textual or character segments which should be treated as 
...
\node[shape=circle,draw=circ!40,line width=4pt, text=circ,fill=red!4,text centered,text width=8pt,inner sep=4pt] at (2,25) {1};

\node[shape=circle,draw=circ,line width=1pt, text=circ,fill=red!4,text centered,text width=8pt,inner sep=4pt] at (2,25) {1};



\node[shape=circle,draw=circ!40,line width=4pt, text=circ,fill=red!4,text centered,text width=8pt,inner sep=4pt] at (12,25) {2};

\node[shape=circle,draw=circ,line width=1pt, text=circ,fill=red!4,text centered,text width=8pt,inner sep=4pt] at (12,25) {2};



\node[shape=circle,draw=circ!40,line width=4pt, text=circ,fill=red!4,text centered,text width=8pt,inner sep=4pt] at (2.3,11) {3};

\node[shape=circle,draw=circ,line width=1pt, text=circ,fill=red!4,text centered,text width=8pt,inner sep=4pt] at (2.3,11) {3};
...
\node [draw=purple!60!red, fill=purple!40!orange, anchor = south west, rounded corners,
text width=13.1cm, inner sep=14pt, xshift=-3pt, yshift=-4pt, font={\fontsize{9}{9}\selectfont}] (ltext)
(s1) at (6.74, 19.9) {{The top part of this figure comes from a 
ProPublica study of communities with exceptionally high 
cancer rates (here, the map is centered near 
East Greenville, PA).  The bottom map shows locations of 
PFAS (Per- and Polyfluoroalkyl Substances) contamination 
in a similar geographic region.}
};


\node [draw=orange!60!black, fill=white, fill=white, anchor = south west, rounded corners,
text width=13cm, inner sep=13pt, font={\fontsize{9}{9}\selectfont}] (ltext-outer)
 at (6.73, 19.8) {{The top part of this figure comes from a 
ProPublica study of communities with exceptionally high 
cancer rates (here, the map is centered near 
East Greenville, PA).  The bottom map shows locations of 
PFAS (Per- and Polyfluoroalkyl Substances) contamination 
...
\node [draw=red!60!black, fill=bgray!49, anchor = south west, rounded corners,
text width=13.4cm, inner sep=13pt, xshift=-2pt, yshift=-5pt, font={\fontsize{9}{9}\selectfont}] (ltext)
(s1) at (6.94, 1.9) {{The specific subject-matter of 
environmental pollution and its relation to cancer 
incidents gives rise to software requirements which, 
ideally, are more specific than basic GIS functionality.  
Each geospatial area can be correlated with both 
cancer diagnoses (potentially with more specific 
tumor-classification data) and reported or measured 
air or ground contamination.  The desire to 
connect these two sources of information 
has implications for context menus, formation 
of API queries, supplemental visualization 
windows, and other coding facets 
suited for domain-specific microcomponent programming.}
};


\node [draw=red!60!black, fill=white, anchor = south west, rounded corners,
text width=13.4cm, inner sep=11pt, font={\fontsize{9}{9}\selectfont}] (ltext-outer)
 at (6.94, 1.8) {{The specific subject-matter of 
environmental pollution and its relation to cancer 
incidents gives rise to software requirements which, 
ideally, are more specific than basic GIS functionality.  
Each geospatial area can be correlated with both 
...
connect these two sources of information 
has implications for context menus, formation 
of API queries, supplemental visual\-ization 
windows, and other coding facets 
suited for domain-specific microcomponent programming.}
};
...
\colorlet{tbgray}{red!70!gray}
\colorlet{tou}{purple!20!orange}
\colorlet{tin}{red!60!black}


\node [draw=tou!30, fill=tbgray!49, anchor = south west, rounded corners,
text width=4.5cm, inner sep=17pt, xshift=-8pt, yshift=-8pt, font={\fontsize{9}{9}\selectfont}] (ltext)
(s1) at (-.3, 4.4) {{Excerpted from OpenHospital manual.
}
};


\node [draw=tin!60, fill=white, anchor = south west, rounded corners,
text width=4.5cm, inner sep=13pt, font={\fontsize{9}{9}\selectfont}] (ltext-outer)
 at (-.39, 4.3) {Excerpted from OpenHospital manual.
};
...
\node [draw=tou!30, fill=tbgray!49, anchor = south west, rounded corners,
text width=4.5cm, inner sep=17pt, xshift=-8pt, yshift=-8pt, font={\fontsize{9}{9}\selectfont}] (ltext)
(s1) at (11.32, 4.1) {{Excerpted from published article.
}
};


\node [draw=tin!60, fill=white, anchor = south west, rounded corners,
text width=4.4cm, inner sep=13pt, font={\fontsize{9}{9}\selectfont}] (ltext-outer)
 at (11.3, 4) {Excerpted from published article.
};
...
(s1) at (3.9, .9) {{Paragraphs where the matches are located 
are also highlighted at their start and end points, plus with sidebars.
}
};
...
text width=7.5cm, inner sep=13pt, font={\fontsize{9}{9}\selectfont}] (ltext-outer)
 at (3.83, .8) {Paragraphs where the matches are located 
are also highlighted at their start and end points, plus with sidebars.
};

\node [draw=ou!30, fill=bgray!49, anchor = south west, rounded corners,
text width=8.8cm, inner sep=17pt, xshift=-8pt, yshift=-8pt, font={\fontsize{9}{9}\selectfont}] (ltext)
...
main entry \\(Harold Garfinkel), with related entries indicated as well.
}
};


\node [draw=in!60, fill=white, anchor = south west, rounded corners,
text width=8.8cm, inner sep=13pt, font={\fontsize{9}{9}\selectfont}] (ltext-outer)
...
\ifnum\value{page}=17{
\AtTextLowerLeft{\raisebox{321pt}{
...
text or image areas.  In this screenshot the user intends to copy the summary 
of an article, so the selection has to include all of the blurb's text 
...
\node [draw=red!60!black, fill=blue, anchor = south west, rounded corners,
text width=5.84cm, inner sep=17pt, xshift=-8pt, yshift=-8pt, font={\fontsize{9}{9}\selectfont}] (ltext)
(s1) at (4.52, 6.7) {{
...
\node [draw=red!60!black, fill=white, anchor = south west, rounded corners,
text width=5.84cm, inner sep=13pt, font={\fontsize{9}{9}\selectfont}] (ltext-outer)
 at (4.47, 6.5) {
...
\ifnum\value{page}=8{
\AtTextLowerLeft{\raisebox{323pt}{
...
\ifnum\value{page}=20{
\AtTextLowerLeft{\hspace{-28pt}\raisebox{-49pt}{
...
text width=5.44cm, inner sep=17pt, xshift=-8pt, yshift=-8pt, font={\fontsize{9}{9}\selectfont}] (ltext)
at (.07, 13.84) {{A simulation via CHASTE (Cancer, Heart And Soft Tissue Environment).  
An indexed grid hosting these simulation files would be able to read 
annotations from a variery of sources in the C++ code, including 
the \textbf{MultipleCryptGeometryBoundaryCondition} class 
(a subtype of \textbf{AbstractCellPopulationBoundaryCondition}, 
which is templated on the count of spatial dimensions in the model) 
that is specific to this model, and the 
\textbf{OffLatticeSimulation} class (similarly templated) 
within the CHASTE core, implementing the basic quantitative 
logic.  

\vspace{4pt}

Likewise, the steps needed to create the 
video sampled and referenced in the publication 
should be identified.  If CHASTE had included a video 
player component, this could be registered as a 
GUI asset (in practice CHASTE videos are derived 
externally via command-line tools, but that 
pipeline could be reimplemented by a host grid). 
A PacTk grid would program a microcomponent 
specifically designed to show CHASTE videos 
and enable context menus on paused movie frames 
to simulation data, parameters, provenance, 
and publication links.  

\vspace{4pt}

Analogous cross-annotation 
schemes could be put into practice for sources 
potentially distributed alongside CHASTE, 
such as libCellML and libSBML.
}
};


\node [draw=fillc!80!cyan, fill=white, anchor = north west, % rounded corners,
text width=5.44cm, inner sep=13pt, font={\fontsize{9}{9}\selectfont}] (ltext-inner)
 at (0, 13.4) {A simulation via CHASTE (Cancer, Heart And Soft Tissue Environment).  
An indexed grid hosting these simulation files would be able to read 
annotations from a variery of sources in the C++ code, including 
the \textbf{MultipleCryptGeometryBoundaryCondition} class 
(a subtype of \textbf{AbstractCellPopulationBoundaryCondition}, 
...
%https://pmc.ncbi.nlm.nih.gov/articles/instance/3597547/bin/pcbi.1002970.s001.zip

\node [draw=red!20!black, fill=white, anchor = south west, rounded corners,
text width=22cm, inner sep=6.5pt, font={\fontsize{9}{9}\selectfont}] (ltext-outer)
 at (-.04, 1.32) {{Sources: ``Chaste: An Open Source C++ Library for Computational
Physiology and Biology'' 
(\href{https://pmc.ncbi.nlm.nih.gov/articles/PMC3597547/pdf/pcbi.1002970.pdf}{https://pmc.ncbi.nlm.nih.gov/articles/PMC3597547/pdf/pcbi.1002970.pdf});
and the supplemental data files at \href{https://pmc.ncbi.nlm.nih.gov/articles/instance/3597547/bin/pcbi.1002970.s001.zip}%
{https://pmc.ncbi.nlm.nih.gov/articles/instance/3597547/bin/pcbi.1002970.s001.zip}.
}
};

\node [draw=bgray!59!yellow, line width=1pt, fill=white, anchor = south west, rounded corners,
text width=22cm, inner sep=6pt, font={\fontsize{9}{9}\selectfont}] (ltext-outer)
 at (-.04, 1.32) {{Sources: ``Chaste: An Open Source C++ Library for Computational
Physiology and Biology'' 
(\href{https://pmc.ncbi.nlm.nih.gov/articles/PMC3597547/pdf/pcbi.1002970.pdf}{https://pmc.ncbi.nlm.nih.gov/articles/PMC3597547/pdf/pcbi.1002970.pdf});
and the supplemental data files at \href{https://pmc.ncbi.nlm.nih.gov/articles/instance/3597547/bin/pcbi.1002970.s001.zip}%
