...
algorithmically generated graphics, and so forth.  In some instances, code packages may be downloaded and run privately, 
...
\newcommand{\counterx}[1]{\vspace{-1em}\textcolor{red!30!black}{#1}}
...
\newcommand{\micf}[1]{\textcolor{blue!30!black}{#1}}

\newcommand{\mic}{\micf{A Microcomponents Alaternative}}
...
maps are an obvious example of cross-disciplinary integration, because 
...
\pseudoIndent{} To visualize statistical correlations between these data categories, 
...
included, although (as in Figure~\ref{fig:car}) researchers can locate related 
...
\p{When implemented as a microcomponent, such a map display would 
...
\p{\counterex{} Several open-source \EMR{} and Hospital 
...
(see the left part of Figure~\ref{fig:oh}) as 
examples, one noteworthy issue is that of 
\textit{internal} dependencies.  Principles of \FAIR{} grids 
should emphasize \textit{external} dependencies, but 
projects should be designed around minimizing internal-dependency 
complications as well.  For example, both FreeMedForms and OpenHospital 
are build around \SQL{} databases.  That's to be expecte, but 
...
application which is essentially a container for those 
...
\p{Where plugins \textit{should} be used is 
...
mechanism from \Qt{}.  In this sense, balancing 
