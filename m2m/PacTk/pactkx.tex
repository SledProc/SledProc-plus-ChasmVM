...
and assign unique ids to each part (see Figure~\ref{fig:NC}).  
For example, segments of text before and after a block quote 
(plus the quote itself) can have distinct ids, even if they 
are part of an encompassing paragraph.

\pseudoIndent{} Given this precision, digital indexes even in PDF documents 
can benefit from paragraph ids.  The bottom part of 
Figure~\ref{fig:NC} shows an index with color-coding to 
...
scroll to the a precise location (top of paragraph or of page).
...
structured objects (such as \textbf{date} and \textbf{proper-name} tags in 
JATS) wherever segments follow standard encodings beyond 
...
Treating such portions of a document as unstructured text material 
...
evolution of practical interventions,\footnote{\hspace*{4pt}Also 
...
to work through the initial research goals on 
purely scientific grounds.  In the \textit{Data Integration} book 
