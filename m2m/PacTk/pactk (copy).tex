...
or vice-versa), or selected words (e.g., last name only), 
...
appropriate for query text seeking locators specific to each entry.
...
Each time a new publiction is finalized, all of its index 
...
repository.  Such meta-indexes can support searches 
...
discursive roles are ignored (e.g., a document may be identified as 
...
\vspace*{5pt}
%\sectnum{1.}
\hspace{2pt}\secttitle{{Data Publishing and Multimedia}}
\vspace*{5pt}
{\semihr{0.26\textwidth}}

\vspace*{5pt}

\p{Contporary data transparency and \FAIRSharing{} 
...
and statistical/data-visualization tools and diagrams).}
...
\pseudoIndent{} Some domain-specific file formats are derived from general-purpose formats 
...
\pseudoIndent{} \lPacTk{} seeks to address these concerns by offering a 
...
take respondibility for deserializing encoded data sets.  Depend 
on the format, the \VM{} instructions might navigate through 
graph-like structures in a standardized profile (like \XML{}) 
or implement and/or integrate with (via postprocessors or 
rule-match callbacks) parsers for custom imput file types.
...
libraries targing a \PacTk{} \VM{}, rather than duplicating 
effort and requiring multiple libraries for different operating 
...
drawn therefrom) as well as reuse.  
...
document, \PacTk{} can be utilized as a code libary by 
...
runtimes allows query evaluators to recgonize basic 
...
fully leveraged published data sets for 
evaluation and re-use.

\pseudoIndent{}  Unfortunately, relying on generic 
software to access published data packages is often 
suboptimal.  For example, \CSV{} files can be 
...
and re-use involves front-end programs that can provide 
...
\pseudoIndent{}  Many data sets, moreover, combine multiple 
data structures.  For example, consider information 
tracking environmental pollutants, such as published 
by the US \EPA{} Toxic Release Inventory (\TRI{}) project.
...
targeting science applications with requisite capbilities 
...
parse-structures; or graphics tools for digital humanities.

\pseudoIndent{}  \lPacTk{} can be beneficial this context 
because a single \PacTk{} plugin could serve as middleware 
on top of which plguins can run that are individualized 
...
courts may let this little girl fie
k -- lisa steinberg horror
k -- state-pol-act- 
Ohel-Agency-Under-Baby-Selling-Probe
